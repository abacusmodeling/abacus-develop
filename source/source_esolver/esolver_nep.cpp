...
void ESolver_NEP::runner(BaseCell& basecell, const int istep)
{
    ...
    if (basecell.kind() == BaseCell::Kind::mdcell)
    {
        ...
        std::vector<double*> position_ptrs(static_cast<std::size_t>(natom), NULL);
        ...
        for (int iat = 0; iat < nowned_atoms; ++iat)
        {
            mutable_owned_atoms[static_cast<std::size_t>(iat)].force.set(force[static_cast<std::size_t>(iat)][0],
                                                                           ...
                                                                           force[static_cast<std::size_t>(iat)][2]);
        }
        ...
        MPI_Allreduce(MPI_IN_PLACE, local_virial, 6, MPI_DOUBLE, MPI_SUM, mdcell.communicator());
#endif
        ...
        nep_virial(2, 2) = local_virial[2] * fact_v;
        nep_virial(0, 1) = nep_virial(1, 0) = local_virial[3] * fact_v;
        nep_virial(0, 2) = nep_virial(2, 0) = local_virial[4] * fact_v;
        nep_virial(1, 2) = nep_virial(2, 1) = local_virial[5] * fact_v;
#endif
...
void ESolver_NEP::type_map(const UnitCell& ucell)
{
    ...
    for (int it = 0; it < ucell.ntype; ++it)
    {
        for (int ia = 0; ia < ucell.atoms[it].na; ++ia)
        {
            atype[static_cast<std::size_t>(iat++)] = md_type_to_nep_type_[static_cast<std::size_t>(it)];
        }
    }
    assert(ucell.nat == iat);
}

void ESolver_NEP::initialize_type_map_(const std::vector<std::string>& type_labels)
{
    ...
    md_type_to_nep_type_.resize(type_labels.size());
    for (std::size_t it = 0; it < type_labels.size(); ++it)
    {
        ...
        md_type_to_nep_type_[it] = found->second;
    }
}
#endif
