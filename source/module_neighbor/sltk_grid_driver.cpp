...
std::vector<std::vector<std::tuple<int, int, ModuleBase::Vector3<int>, ModuleBase::Vector3<double>>>> Grid_Driver::Find_atoms(const UnitCell& ucell_in)
{
	...
	for(size_t iat=0; iat<ucell_in.nat; iat++)
    {
		...
		std::vector<std::tuple<int, int, ModuleBase::Vector3<int>, ModuleBase::Vector3<double>>> adjss;
		const int offset = this->Locate_offset(ucell_in, tau, it, ia);
		...
		for (int i = 0;i < adj_num1;i++)
		{
			...
			const int offset_i = as->offset[i];
			// AdjacentSet::getBox(as->box[i], box[i].x, box[i].y, box[i].z);
			ModuleBase::Vector3<int> box_ad;		
			ModuleBase::Vector3<double> tau_ad;
			AdjacentSet::getBox(as->box[i], box_ad.x,box_ad.y, box_ad.z);
			...
			// natom : get the adjacent atom index in this type
			//----------------------------------------------------------
			const size_t it_ad  = this->atomlink[offset_i].fatom.getType();
			const size_t ia_ad = this->atomlink[offset_i].fatom.getNatom();
			...
			if (expand_flag)
			{
				...
				// NAME : Distance
				//----------------------------------------------------------
				tau_ad.x = this->atomlink[offset_i].fatom.x();
				tau_ad.y = this->atomlink[offset_i].fatom.y();
				tau_ad.z = this->atomlink[offset_i].fatom.z();
			...
			// NAME : Distance
			//----------------------------------------------------------
				tau_ad = Calculate_adjacent_site(offset_i,
								vec1[0], vec2[0], vec3[0],
								vec1[1], vec2[1], vec3[1],
								vec1[2], vec2[2], vec3[2],
								box_ad.x, box_ad.y, box_ad.z);

				if (test_grid_driver > 1)
				{
					if(i<100)
					{
						const double distance = this->Distance(this->atomlink[offset],tau_ad);
						/*
											GlobalV::ofs_running << std::setw(10) << adjacent_tau[i].x
											<< std::setw(10) << adjacent_tau[i].y
											<< std::setw(10) << adjacent_tau[i].z
			...
			adjss.push_back(std::make_tuple(it_ad, ia_ad, box_ad, tau_ad));
		}// end adjacent number
		
		// the last one is the atom itself.
		int atom_it = this->atomlink[offset].fatom.getType();
		int atom_ia = this->atomlink[offset].fatom.getNatom();
		ModuleBase::Vector3<double> atom_tau;
		atom_tau.x = this->atomlink[offset].fatom.x();
		
		atom_tau.y = this->atomlink[offset].fatom.y();
		atom_tau.z = this->atomlink[offset].fatom.z();
		// mohan add 2010-07-01
		ModuleBase::Vector3<int> atom_box;
		atom_box.x = 0;
		atom_box.y = 0;
		atom_box.z = 0;
		adjss.push_back(std::make_tuple(atom_it, atom_ia, atom_box, atom_tau));
