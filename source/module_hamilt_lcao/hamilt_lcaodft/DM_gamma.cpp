                                          ...
                                          const int& istep)
{
    ...
    if (GlobalV::OUT_LEVEL != "m")
        ModuleBase::GlobalFunc::OUT(GlobalV::ofs_running, "lgd_last", lgd_last);
    if (GlobalV::OUT_LEVEL != "m")
    ...
    if (lgd_now > 0)
    {
        ...
        if (GlobalV::OUT_LEVEL != "m")
            GlobalV::ofs_running << " allocate DM , the dimension is " << lgd_now << std::endl;
    }
    else if (lgd_now == 0)
    {
        this->init_DM = false;
...
#ifdef __MPI
    this->dm2g.setAlltoallvParameter(this->ParaV->comm_2D,
                                     GlobalV::NLOCAL,
...
void Local_Orbital_Charge::cal_dk_gamma_from_2D_pub()
{
    ...
    this->dm2g.cal_dk_gamma_from_2D(this->dm_gamma,
                                    this->DM,
                                    GlobalV::NSPIN,
                                    GlobalV::NLOCAL,
                                    this->lgd_now,
