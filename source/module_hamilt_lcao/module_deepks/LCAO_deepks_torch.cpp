...
// 3. cal_gvx : gvx is used for training with force label, which is gradient of
// descriptors,
...
//       this is the term V(D) that enters the expression H_V_delta =
//       |alpha>V(D)<alpha| caculated using torch::autograd::grad
// 8. check_gedm : prints gedm for checking
// 9. cal_orbital_precalc : orbital_precalc is usted for training with orbital
// label,
//                          which equals gvdm * orbital_pdm_shell,
//                          orbital_pdm_shell[1,Inl,nm*nm] = dm_hl * overlap *
//                          overlap
        ...
        for (int I0 = 0; I0 < atom0->na; I0++) {
            ...
            for (int L0 = 0; L0 <= orb.Alpha[0].getLmax(); ++L0) {
                for (int N0 = 0; N0 < orb.Alpha[0].getNchi(L0); ++N0) {
                    ...
                    for (int m1 = 0; m1 < nm;
                         ++m1) // m1 = 1 for s, 3 for p, 5 for d
                    {
                        for (int m2 = 0; m2 < nm;
                             ++m2) // m1 = 1 for s, 3 for p, 5 for d
                        {
                ...
                for (int ad2 = 0; ad2 < GridD.getAdjacentNum() + 1; ad2++) {
                    ...
                    for (int icol = 0; icol < col_size; icol++) {
                        const double* col_ptr
                            = this->nlm_save[iat][ad2][col_indexes[icol]][0]
                                  .data();
                        for (int i = 0; i < trace_alpha_size; i++) {
    ...
    for (int T0 = 0; T0 < ucell.ntype; T0++) {
        ...
        for (int I0 = 0; I0 < atom0->na; I0++) {
            ...
            for (int ad1 = 0; ad1 < GridD.getAdjacentNum() + 1; ++ad1) {
                ...
                for (int irow = 0; irow < row_size; irow++) {
                    const double* row_ptr
                        = this->nlm_save_k[iat][key_1][row_indexes[irow]][0]
                              .data();
                    for (int i = 0; i < trace_alpha_size; i++) {
                        s_1t[i * row_size + irow] = row_ptr[trace_alpha_row[i]];
                    }
                ...
                for (int ad2 = 0; ad2 < GridD.getAdjacentNum() + 1; ad2++) {
                    ...
                    std::vector<double> dm_array(row_size * nks * col_size,
                                                 0.0);
                    ...
                    for (int ik = 0; ik < nks; ik++) {
                        hamilt::AtomPair<double> dm_pair(ibt1,
                                                         ...
                                                         (dR2 - dR1).z,
                                                         pv);
                        dm_pair.allocate(&dm_array[ik * row_size * col_size],
                                         0);
                        const double arg
