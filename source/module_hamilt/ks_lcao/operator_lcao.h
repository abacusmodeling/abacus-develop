...
class OperatorLCAO : public Operator<T>
{
    ...
    {
        if(this->allocated_smatrix) delete[] this->smatrix_k;
    };
    ...
    */
    ...
    Gamma_only case (T = double), SK would not changed during one SCF loop, a template triangle matrix SK_temp is used for accelerating.
    General case (T = std::complex<double>), only pointers of HK and SK saved in OperatorLCAO 
    */
