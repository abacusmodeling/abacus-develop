...
void ModuleIO::write_bands(const Input_para& inp, const ModuleBase::matrix& ekb, const K_Vectors& kv)
{
    ...
    if (inp.out_band[0])
    {
        ...
        const Parallel::ParaBandOutput band_output(ekb.nc, inp.nbands, Parallel::make_band_world());
...
void ModuleIO::nscf_bands(const int& is,
                          ...
                          const K_Vectors& kv)
{
    ...
    */
...
#ifdef __MPI
    const int band_group = Parallel::make_band_world().rank();
    ...
    for (int ik = 0; ik < nkstot_np; ik++)
    {
        ...
        // Collinear spin channels occupy consecutive blocks of physical k-points.
        const int spin_k = ik + is * nks_np;
        if (kv.isk[spin_k] == is)
        {
            ofs << FmtCore::format("%4d", ik + 1);
            int width = precision + 4;
            ...
            {
                ofs << FmtCore::format(fmtstr.c_str(), (ekb(spin_k, ibnd) - fermie) * ModuleBase::Ry_to_eV);
            }
            ofs << std::endl;
        }
    }
