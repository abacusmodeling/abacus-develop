...
bool Update_input::Read(const string &fn)
{
    ...
    while (ifs.good())
    {
        ...
        if (strcmp("force", word) == 0)
        {
			...
			if(force!=FORCE)
			{
				this->change(ofs_warning,"FORCE",FORCE, force);
    			FORCE = this->force;
			}
        }
		// 2
        else if (strcmp("force_thr", word) == 0)
        {
			...
			if(force_thr!=FORCE_THR)
			{
				this->change(ofs_warning,"forth_thr(Ry/Bohr)",FORCE_THR,force_thr);
    			FORCE_THR = this->force_thr;
			}
        }
		// 2
        else if (strcmp("force_thr_ev", word) == 0)
        {
			...
			if(force_thr!=FORCE_THR)
			{
				this->change(ofs_warning,"force_thr(Ry/Bohr)",FORCE_THR,force_thr);
    			FORCE_THR = this->force_thr;
			}
        }
		// 3
        ...
        else if (strcmp("dr2", word) == 0)
        {
			...
			if(dr2!=DRHO2)
			{
				this->change(ofs_warning,"dr2",DRHO2,dr2);
    			DRHO2 = this->dr2;
			}
        }
		// 5
        else if (strcmp("niter", word) == 0)
        {
			...
			if(niter!=NITER)
			{
				this->change(ofs_warning,"niter",NITER,niter);
    			NITER = this->niter;
			}
        }
		// 6
        else if (strcmp("nstep", word) == 0)
        {
			...
			if(nstep!=NSTEP)
			{
				this->change(ofs_warning,"nstep",NSTEP,nstep);
    			NSTEP = this->nstep;
			}
        }
		// 7
        ...
        else if (strcmp("printe", word) == 0)
        {
			...
			if(printe!=en.printe)
			{
				this->change(ofs_warning,"printe",en.printe,printe);
        ...
        else if (strcmp("charge_extrap", word) == 0)//xiaohui modify 2015-02-01
        {
			...
			{
				this->change(ofs_warning,"extra_pot",pot.extra_pot,charge_extrap);
				pot.extra_pot = this->charge_extrap;
				if(out_dm==0) 
				{
					out_dm = 10000;
#ifdef __FP
					this->change(ofs_warning,"out_dm",LOC.out_dm, out_dm);
					LOC.out_dm=out_dm;
#endif
				}
			}
        ...
        else if (strcmp("out_dos", word) == 0)
        {
			...
			{
				this->change(ofs_warning,"out_dos",en.out_dos,out_dos);
