...
void ElecStatePW::rhoBandK(const psi::Psi<std::complex<double>>& psi)
{
    ...
    if(GlobalV::NSPIN==4) wfcr_another_spin.resize(this->charge->nrxx);
    ...
    if(GlobalV::NSPIN==4)
    {
        ...
        for (int ibnd = 0;ibnd < nbands;ibnd++)
        {
            ...
            ///
            if(this->wg(ik, ibnd)<ModuleBase::threshold_wg) continue;
            ...
            if(GlobalV::DOMAG){
                for(int ir= 0;ir<this->charge->nrxx;ir++)
                {
                    this->charge->rho[1][ir] += w1 * 2.0 * (wfcr[ir].real()* wfcr_another_spin[ir].real()
                        + wfcr[ir].imag()* wfcr_another_spin[ir].imag());
                    this->charge->rho[2][ir] += w1 * 2.0 * (wfcr[ir].real()* wfcr_another_spin[ir].imag()
            ...
            {
                for(int ir = 0;ir<this->charge->nrxx;ir++) this->charge->rho[is][ir] = 0;
            }
        }
    }
    else
    for (int ibnd = 0;ibnd < nbands;ibnd++)
    {
        ...
        if (XC_Functional::get_func_type() == 3)
        {
            for (int j=0; j<3; j++)
            {
                ...
                for (int ig = 0;ig < npw ; ig++)
                {
                    double fact = this->basis->get_GPlusK_cartesian_projection(ik, igk[ig] ,j) * GlobalC::ucell.tpiba;
