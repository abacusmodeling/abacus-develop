...
#endif
        for (int isk = 0;isk < this->kv.get_nks();++isk)DM_trans.set_DMK_pointer(isk, dm_trans_2d[isk].data<double>());
        DM_trans.cal_DMR();
...
void LR::LR_Spectrum<std::complex<double>>::oscillator_strength()
{
    ...
    for (int istate = 0;istate < X.get_nbands();++istate)
    {
        ...
        //1. transition density 
#ifdef __MPI
        std::vector<container::Tensor>  dm_trans_2d = cal_dm_trans_pblas(X, this->pX, this->psi_ks, this->pc, this->naos, this->nocc, this->nvirt, this->pmat, /*renorm_k=*/false, this->nspin);
        // if (this->tdm_sym) for (auto& t : dm_trans_2d) LR_Util::matsym(t.data<T>(), naos, pmat);
#else
        std::vector<container::Tensor>  dm_trans_2d = cal_dm_trans_blas(X, this->psi_ks, this->nocc, this->nvirt,/*renorm_k=*/false, this->nspin);
        // if (this->tdm_sym) for (auto& t : dm_trans_2d) LR_Util::matsym(t.data<T>(), naos);
#endif
        for (int isk = 0;isk < this->kv.get_nks();++isk)DM_trans.set_DMK_pointer(isk, dm_trans_2d[isk].data<std::complex<double>>());
...
template<typename T>
void LR::LR_Spectrum<T>::optical_absorption(const std::vector<double>& freq, const double eta)
{
...
template<typename T>
void LR::LR_Spectrum<T>::transition_analysis()
{
