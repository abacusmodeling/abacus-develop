...
// ----------------------------------------------------------------------
	...
	// allocate flag for each FFT grid.
    int* symflag = new int[fftnx*fftny*fftnz];
    int* isymflag = new int[fftnx*fftny*fftnz];
    //std::cout << "nrotk is : " << nrotk <<std::endl;
    //int table_xyz[fftnx*fftny*fftnz][48];
    int(*table_xyz)[48] = new int[fftnx*fftny*fftnz][48];
    //std::cout << "could build table_xyz" << std::endl;
    int* table_column = new int[fftnx*fftny*fftnz];
            ...
            for (int k = 0; k< fftnz; ++k)
            {
                ...
                if (symflag[ixyz0] == -1)
                {
                    ...
                    for (int isym = 0; isym < nrotk; ++isym)
                    {
                        ...
                        //std::cout << "I am searching grids in one group" <<std::endl;
                    }
...
ModuleBase::timer::tick("Symmetry","group fft grids");
//std::cout << "group_index in grouping is : " << group_index <<std::endl;
...
for (int g_index = 0; g_index < group_index; g_index++)
{
    ...
    for (int c_index = 0; c_index < table_column[g_index]; ++c_index)
    {
                ...
                {
                    //std::cout << "working on g_index : " << g_index << std::endl;
