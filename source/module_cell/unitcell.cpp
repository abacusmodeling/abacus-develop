...
void UnitCell::read_pseudo(std::ofstream& ofs)
{
    ...
    if (GlobalV::MY_RANK == 0)
    {
        ...
        if (GlobalV::out_element_info)
        {
            ...
            for (int it = 0; it < ntype; it++)
            {
                Atom* atom = &atoms[it];
                std::stringstream ss;
                ss << GlobalV::global_out_dir << atom->label << "/" << atom->label << ".NONLOCAL";
                std::ofstream ofs(ss.str().c_str());
    ...
    if (GlobalV::MY_RANK == 0)
    {
        ...
        for (int it = 0; it < ntype; it++)
        {
            if (ModuleBase::MinZval.find(atoms[it].ncpp.psd) != ModuleBase::MinZval.end())
            {
                if (atoms[it].ncpp.zv > ModuleBase::MinZval.at(atoms[it].ncpp.psd))
                {
                    ...
                    std::cout << " Warning: the number of valence electrons in pseudopotential > "
                              << ModuleBase::MinZval.at(atoms[it].ncpp.psd);
                    std::cout << " for " << atoms[it].ncpp.psd << ": " << ModuleBase::EleConfig.at(atoms[it].ncpp.psd)
                              << std::endl;
                    ofs << " Warning: the number of valence electrons in pseudopotential > "
                        << ModuleBase::MinZval.at(atoms[it].ncpp.psd);
        ...
        if (abtype > 0)
        {
            std::cout << " Pseudopotentials with additional electrons can yield (more) accurate outcomes, but may be "
                         "less efficient."
                      << std::endl;
            std::cout << " If you're confident that your chosen pseudopotential is appropriate, you can safely ignore "
                         "this warning."
                      << std::endl;
            std::cout << "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
                         "%%%%%%%%%%%%\n"
                      << std::endl;
            ofs << " Pseudopotentials with additional electrons can yield (more) accurate outcomes, but may be less "
                   "efficient."
                << std::endl;
            ofs << " If you're confident that your chosen pseudopotential is appropriate, you can safely ignore this "
                   "warning."
                << std::endl;
            ofs << "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
                   "%%%%%%%";
...
void UnitCell::cal_nwfc(std::ofstream& log)
{
    ...
    */
    ...
    if ((GlobalV::BASIS_TYPE == "lcao") || (GlobalV::BASIS_TYPE == "lcao_in_pw")
        || ((GlobalV::BASIS_TYPE == "pw") && (GlobalV::psi_initializer) && (GlobalV::init_wfc.substr(0, 3) == "nao")
                    ...
                    for (int a = -1; a < 2; a++)
                    {
                        for (int b = -1; b < 2; b++)
                        {
                            for (int c = -1; c < 2; c++)
                            {
                                ...
                                double bond_length
                                    = sqrt(pow((x2 - x1) * this->a1.x + (y2 - y1) * this->a2.x + (z2 - z1) * this->a3.x,
                                               2)
                                           + pow((x2 - x1) * this->a1.y + (y2 - y1) * this->a2.y
                                                     + (z2 - z1) * this->a3.y,
                                                 2)
                                           + pow((x2 - x1) * this->a1.z + (y2 - y1) * this->a2.z
                                                     + (z2 - z1) * this->a3.z,
                                                 2))
                                      * this->lat0;

                                if (bond_length < covalent_length * factor
                                    || bond_length < covalent_length * warning_coef)
                                {
                                    errorlog.setf(std::ios_base::fixed, std::ios_base::floatfield);
                                    errorlog << std::setw(3) << ia1 + 1 << "-th " << std::setw(3)
                                             << this->atoms[it1].label << ", ";
                                    errorlog << std::setw(3) << ia2 + 1 << "-th " << std::setw(3)
                                             << this->atoms[it2].label;
                                    errorlog << " (cell:" << std::setw(2) << a << " " << std::setw(2) << b << " "
                                             << std::setw(2) << c << ")";
...
void UnitCell::cal_nelec(double& nelec)
{
    ...
    if (GlobalV::nelec_delta != 0)
    {
        ModuleBase::GlobalFunc::OUT(
            GlobalV::ofs_running,
            "nelec_delta is NOT zero, please make sure you know what you are doing! nelec_delta: ",
            GlobalV::nelec_delta);
