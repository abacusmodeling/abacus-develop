...
TEST_F(FIRE_test, first_half)
{
    ...
    EXPECT_NEAR(mdrun->pos[0].x, 9.995455294044568, doublethreshold);
    EXPECT_NEAR(mdrun->pos[0].y, 0.003264683323249327, doublethreshold);
    EXPECT_NEAR(mdrun->pos[0].z, 9.9994784290476932, doublethreshold);
    EXPECT_NEAR(mdrun->pos[1].x, 5.2052136746814073, doublethreshold);
    EXPECT_NEAR(mdrun->pos[1].y, 5.19405131115556, doublethreshold);
    EXPECT_NEAR(mdrun->pos[1].z, 0.0035886062145122004, doublethreshold);
    EXPECT_NEAR(mdrun->pos[2].x, 5.0947593079696478, doublethreshold);
    EXPECT_NEAR(mdrun->pos[2].y, 0.00048706739346586134, doublethreshold);
    EXPECT_NEAR(mdrun->pos[2].z, 4.9979870945593055, doublethreshold);
    EXPECT_NEAR(mdrun->pos[3].x, 0.0045717233044145923, doublethreshold);
    EXPECT_NEAR(mdrun->pos[3].y, 5.3021969381277305, doublethreshold);
    EXPECT_NEAR(mdrun->pos[3].z, 4.9989458701784848, doublethreshold);
