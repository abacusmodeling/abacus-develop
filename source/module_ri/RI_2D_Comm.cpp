...
-> std::vector<std::tuple<std::set<TA>, std::set<TA>>>
{
	...
	for(int iwt0_2D=0; iwt0_2D<pv.MatrixInfo.row_set.size(); ++iwt0_2D)
	{
		const int iwt0 = pv.MatrixInfo.row_set[iwt0_2D];
	...
	for(int iwt1_2D=0; iwt1_2D<pv.MatrixInfo.col_set.size(); ++iwt1_2D)
	{
		const int iwt1 = pv.MatrixInfo.col_set[iwt1_2D];
