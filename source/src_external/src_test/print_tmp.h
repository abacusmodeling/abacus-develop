...
void print_matrixess(const std::map<size_t,std::map<size_t,std::map<Abfs::Vector3_Order<double>,std::weak_ptr<matrix>>>> &ms, std::ofstream &ofs)
{
...
void print_matrixess(const std::map<size_t,std::map<size_t,std::map<Abfs::Vector3_Order<int>,std::shared_ptr<matrix>>>> &ms, std::ofstream &ofs)
{
	for( const auto & c1 : ms )
		for( const auto & c2 : c1.second )
			for( const auto & c3 : c2.second )
