...
void Parallel_Kpoints::divide_pools(void)
{
    ...
    int i=0;
    int j=0;
    if (GlobalV::NPROC<GlobalV::PW_KPAR)
    {
        std::cout<<"\n GlobalV::NPROC=" << GlobalV::NPROC << " PW_KPAR=" << GlobalV::PW_KPAR;
        std::cout<<"Error : Too many pools !"<<std::endl;
        exit(0);
    }
    //if(GlobalC::kv.nkstot<GlobalV::PW_KPAR) std::cout<<"Error !"<<std::endl;

    // (1) per process in each pool
    GlobalV::NPROC_IN_POOL = GlobalV::NPROC/GlobalV::PW_KPAR;
    if (GlobalV::MY_RANK < (GlobalV::NPROC%GlobalV::PW_KPAR)*(GlobalV::NPROC_IN_POOL+1))
    {
        GlobalV::NPROC_IN_POOL++;
    }

    // (2) To know how many process in pool j.
    nproc_pool = new int[GlobalV::PW_KPAR];
    ModuleBase::GlobalFunc::ZEROS(nproc_pool, GlobalV::PW_KPAR);
    ...
    startpro_pool = new int[GlobalV::PW_KPAR];
    ModuleBase::GlobalFunc::ZEROS(startpro_pool, GlobalV::PW_KPAR);
    for (i=1; i<GlobalV::PW_KPAR; i++)
    {
        startpro_pool[i]=startpro_pool[i-1]+nproc_pool[i-1];
    }

    // use 'GlobalV::MY_RANK' to know 'GlobalV::MY_POOL'.
    for (i=0; i<GlobalV::PW_KPAR; i++)
    {
    ...
    {
        assert(GlobalV::NPROC%GlobalV::PW_KPAR == 0);
...
{
    delete[] nks_pool;
    this->nks_pool = new int[GlobalV::PW_KPAR];
    ModuleBase::GlobalFunc::ZEROS(nks_pool, GlobalV::PW_KPAR);

    const int nks_ave = nkstot/GlobalV::PW_KPAR;
    const int remain = nkstot%GlobalV::PW_KPAR;

    //GlobalV::ofs_running << "\n nkstot = " << nkstot;
    //GlobalV::ofs_running << "\n GlobalV::PW_KPAR = " << GlobalV::PW_KPAR;
    //GlobalV::ofs_running << "\n nks_ave = " << nks_ave;
...
{
    delete[] startk_pool;
    startk_pool = new int[GlobalV::PW_KPAR];
    //const int remain = nkstot%GlobalV::PW_KPAR;

    startk_pool[0] = 0;
    for (int i=1; i<GlobalV::PW_KPAR; i++)
    {
