...
using namespace std;
TEST_F(PWTEST,test1_2_2f)
{
    cout<<"dividemthd 1, gamma_only: off, float precision, check fft between double and complex"<<endl;
    ...
    for(int ixy = 0 ; ixy < fftnx * fftny ; ++ixy)
    {
        ...
        {
            EXPECT_NEAR(tmp[ixy * fftnz + startiz + iz].real(),rhor[ixy*nplane+iz],1e-4);
            EXPECT_NEAR(tmp[ixy * fftnz + startiz + iz].real(),((float*)rhogr)[ixy*nplane+iz],1e-4);
        }
    }
    ...
    pwtest.real2recip((float*)rhogr,rhogr);//check in-place transform
