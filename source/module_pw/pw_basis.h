...
public:
    ...
    void real2recip(const double * in, std::complex<double> * out); //in:(nplane,nx*ny)  ; out(nz, ns)
    void real2recip(const std::complex<double> * in, std::complex<double> * out); //in:(nplane,nx*ny)  ; out(nz, ns)
    void recip2real(const std::complex<double> * in, double *out); //in:(nz, ns)  ; out(nplane,nx*ny)
    void recip2real(const std::complex<double> * in, std::complex<double> * out); //in:(nz, ns)  ; out(nplane,nx*ny)

#ifdef __MIX_PRECISION
    void real2recip(const float * in, std::complex<float> * out); //in:(nplane,nx*ny)  ; out(nz, ns)
    void real2recip(const std::complex<float> * in, std::complex<float> * out); //in:(nplane,nx*ny)  ; out(nz, ns)
    void recip2real(const std::complex<float> * in, float *out); //in:(nz, ns)  ; out(nplane,nx*ny)
    void recip2real(const std::complex<float> * in, std::complex<float> * out); //in:(nz, ns)  ; out(nplane,nx*ny)
#endif
protected:
