...
void PW_Basis::distribution_method2()
{
    ...
    if (poolrank == 0)
    {
        ...
        this->divide_sticks2(tot_npw, tot_nst, st_i, st_j, st_length, npw_per, nst_per, istot2ip);
        this->get_istot2ixy2(tot_nst, st_i, st_j, istot2ip);
        ...
        this->divide_pw2(tot_npw, gg_global, gdirect_global, gg2D, gdirect2D);
    ...
    // {
    //     gcar[ig] = gdirect[ig] * this->G;
    // }

    // (6) Construct ig2isz and is2ixy. 
    this->get_ig2isz_is2ixy2(tot_nst, st_i, st_j, st_bottom, st_length, istot2ip);
...
//     st_sorted_index[0] = 0;
//     ModuleBase::heapsort(tot_nst, temp_st_length, st_sorted_index); // sort st_* in the order of length decreasing.
...
//         st_bottom[istot] = temp_st_bottom[st_sorted_index[istot]];
//     }
...
)
{
    ...
    {
        int ip = istot2ip[is];
        this->istot2ixy[this->startnsz_per[ip] / this->nz + st_move[ip]] = st_i[is] * ny + st_j[is];
        this->ixy2istot[st_i[is] * ny + st_j[is]] = this->startnsz_per[ip] / this->nz + st_move[ip];
        st_move[ip]++;
    }
    delete[] st_move;
    return;
...
//
void PW_Basis::get_ig2isz_is2ixy2(
    const int tot_nst,  // total number of sticks.
    int* st_i,          // x or x + nx (if x < 0) of stick.
    int* st_j,          // y or y + ny (if y < 0) of stick.
    int* st_bottom,     // minimum z of stick, stored in 1d array with tot_nst elements.
...
)
{
    ...
    for (int is = 0; is < tot_nst; is++)
    {
        int ix = st_i[is];
        int iy = st_j[is];
        if (ix >= int(nx/2) + 1 && this->gamma_only == false) ix -= nx;
        ...
        if (istot2ip[is] == poolrank)
        {
            ...
            for (int iz = zstart; iz < zstart + st_length[is]; iz++)
            {
                ...
                this->gg[pw_filled] = f * (GGT * f);
                this->gdirect[pw_filled] = f;
                this->gcar[pw_filled] = f * this->G;
                pw_filled++;
            }
            this->is2ixy[st_move] = st_i[is] * ny + st_j[is];
            st_move++;
        }
