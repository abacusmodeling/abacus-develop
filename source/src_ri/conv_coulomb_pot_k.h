...
class Conv_Coulomb_Pot_K {
  public:
    ...
    template <typename T>
    static T cal_orbs_ccp(const T &orbs, const Ccp_Type &ccp_type,
                          const std::map<std::string, double> &parameter,
                          const double rmesh_times);

    template <typename T>
    static T cal_orbs_ccp_rmesh(const T &orbs, const Ccp_Type &ccp_type,
                                const std::map<std::string, double> &parameter,
  ...
  private:
    template <typename T>
    static double get_rmesh_proportion(const T &orbs,
                                       const double psi_threshold);
