...
cout<<"cal_Vs\t"<<iat1<<"\t"<<iat2<<"\t"<<box2<<endl;
		...
		vector<ComplexMatrix> Vkps( kv.nks/GlobalV::NSPIN, {Vs[iat1][iat2][{0,0,0}]->nr, Vs[iat1][iat2][{0,0,0}]->nc} );
		for( size_t ik=0; ik!=kv.nks/GlobalV::NSPIN; ++ik )
			for( const Vector3_Order<int> &box2 : Coulomb_potential_boxes )
		...
		for( const Vector3_Order<int> &box2 : Born_von_Karman_boxes )
		{
			...
			for( size_t ik=0; ik!=kv.nks/GlobalV::NSPIN; ++ik )
				Vps_tmp += Vkps[ik] * kv.wk[ik]*(0.5*GlobalV::NSPIN) * exp(TWO_PI*IMAG_UNIT*(kv.kvec_c[ik]*(box2*ucell.latvec)));
