...
public:
	...
	// V[it1,0][it2,R]
	static std::shared_ptr<matrix> DPcal_V( 
		const size_t &it1, 
		const size_t &it2, 
		const Vector3_Order<double> &R, 												// unit: ucell.lat0
		const Exx_Abfs::Matrix_Orbs11 &m_abfs_abfs,
...
//	static std::map<size_t,std::vector<Abfs::Vector3_Order<int>>> get_adjs( const size_t &T, const Vector3<double> &tau_cartesian );
	...
	static std::shared_ptr<matrix> cal_I( const std::shared_ptr<matrix> &m );
	static std::vector<std::vector<std::shared_ptr<matrix>>> cal_I( const std::vector<std::vector<std::shared_ptr<matrix>>> &ms );
