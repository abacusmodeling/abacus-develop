...
// Peize Lin test
Exx_Lcao::Exx_Lcao(const Exx_Global::Exx_Info &info_global) : kmesh_times(4), info(info_global)
{
    auto test_gemm = []() {
        ...
        auto print_matrix
            = [](const ModuleBase::matrix &m1, const ModuleBase::matrix &m2, const ModuleBase::matrix &m3) {
        ...
        }
        {
            ...
            BlasConnector::gemm('N', 'N', m3.nr, m3.nc, m1.nc, 1, m1.c, m1.nc, m2.c, m2.nc, 0, m3.c, m3.nc);
            print_matrix(m1, m2, m3);
        }
        {
            const ModuleBase::matrix m1 = transpose(init_matrix(1, 3, 10)), m2 = init_matrix(3, 2, 0);
            ModuleBase::matrix m3(m1.nc, m2.nc);
            BlasConnector::gemm('T', 'N', m3.nr, m3.nc, m1.nr, 1, m1.c, m1.nc, m2.c, m2.nc, 0, m3.c, m3.nc);
            print_matrix(m1, m2, m3);
        }
        {
            const ModuleBase::matrix m1 = transpose(init_matrix(1, 3, 10)), m2 = init_matrix(3, 2, 0);
            ModuleBase::matrix m3(m1.nc, m2.nc);
            BlasConnector::gemm('N', 'T', m3.nr, m3.nc, m1.nr, 1, m1.c, m1.nc, m2.c, m2.nc, 0, m3.c, m3.nc);
            print_matrix(m1, m2, m3);
        }
        {
            const ModuleBase::matrix m1 = init_matrix(1, 3, 10), m2 = transpose(init_matrix(3, 2, 0));
            ModuleBase::matrix m3(m1.nr, m2.nr);
            BlasConnector::gemm('N', 'T', m3.nr, m3.nc, m1.nc, 1, m1.c, m1.nc, m2.c, m2.nc, 0, m3.c, m3.nc);
            print_matrix(m1, m2, m3);
        }
        {
            const ModuleBase::matrix m1 = init_matrix(1, 3, 10), m2 = transpose(init_matrix(3, 2, 0));
            ModuleBase::matrix m3(m1.nr, m2.nr);
            BlasConnector::gemm('T', 'N', m3.nr, m3.nc, m1.nc, 1, m1.c, m1.nc, m2.c, m2.nc, 0, m3.c, m3.nc);
            print_matrix(m1, m2, m3);
        }
    };
...
void Exx_Lcao::init()
{
    ...
    auto test_rk = []() {
        ...
        auto pr_orb_all = [&pr_orb](const std::string &file, const Numerical_Orbital_Lm &orb) {
            ...
            orb_psi_T.set_orbital_info(orb.getLabel(),
                                       orb.getType(),
                                       orb.getL(),
                                       orb.getChi(),
                                       orb.getNr(),
                                       ...
                                       true,
                                       GlobalV::CAL_FORCE);
            orb_psi_F.set_orbital_info(orb.getLabel(),
                                       orb.getType(),
                                       orb.getL(),
                                       orb.getChi(),
                                       orb.getNr(),
                                       ...
                                       false,
                                       GlobalV::CAL_FORCE);
            orb_psif_T.set_orbital_info(orb.getLabel(),
                                        orb.getType(),
                                        orb.getL(),
                                        orb.getChi(),
                                        orb.getNr(),
                                        ...
                                        true,
                                        GlobalV::CAL_FORCE);
            orb_psik_T.set_orbital_info(orb.getLabel(),
                                        orb.getType(),
                                        orb.getL(),
                                        orb.getChi(),
                                        orb.getNr(),
                                        ...
                                        true,
                                        GlobalV::CAL_FORCE);
            orb_psik2_T.set_orbital_info(orb.getLabel(),
                                         orb.getType(),
                                         orb.getL(),
                                         orb.getChi(),
                                         orb.getNr(),
        ...
        auto pr_orb_all_kmesh
            = [&pr_orb](const std::string &file, const Numerical_Orbital_Lm &orb, const int kmesh_times) {
                  ...
                  orb_psi_T.set_orbital_info(orb.getLabel(),
                                             ...
                                             true,
                                             GlobalV::CAL_FORCE);
                  orb_kmesh.set_orbital_info(orb_psi_T.getLabel(),
                                             orb_psi_T.getType(),
                                             orb_psi_T.getL(),
                                             orb_psi_T.getChi(),
                                             orb_psi_T.getNr(),
        ...
        if (false)
        {
            for (int T = 0; T != GlobalC::ORB.get_ntype(); ++T)
            {
                for (int L = 0; L <= GlobalC::ORB.Phi[T].getLmax(); ++L)
                {
                    ...
                    {
                        pr_orb_all("orb_" + ModuleBase::GlobalFunc::TO_STRING(T) + "_"
                                       + ModuleBase::GlobalFunc::TO_STRING(L) + "_"
                                       + ModuleBase::GlobalFunc::TO_STRING(N),
        ...
        else
        {
            for (int T = 0; T != GlobalC::ORB.get_ntype(); ++T)
            {
                for (int L = 0; L <= GlobalC::ORB.Phi[T].getLmax(); ++L)
                {
                    for (int N = 0; N != GlobalC::ORB.Phi[T].getNchi(L); ++N)
                    {
                        pr_orb_all_kmesh("orb_" + ModuleBase::GlobalFunc::TO_STRING(T) + "_"
                                             + ModuleBase::GlobalFunc::TO_STRING(L) + "_"
                                             + ModuleBase::GlobalFunc::TO_STRING(N),
                                         GlobalC::ORB.Phi[T].PhiLN(L, N),
                                         5);
    ...
    auto test_exp = [&]() {
        ...
        for (size_t ik = 0; ik != GlobalC::kv.nks; ++ik)
        {
            for (size_t i = 0; i != boxes.size(); ++i)
            {
                std::cout << exp(-ModuleBase::TWO_PI * ModuleBase::IMAG_UNIT
                                 * (GlobalC::kv.kvec_c[ik] * (boxes[i] * GlobalC::ucell.latvec)))
                          << "\t";
            }
            std::cout << std::endl;
        }
        ...
        for (size_t ik = 0; ik != GlobalC::kv.nks; ++ik)
        {
            ...
            {
                std::cout << exp(-ModuleBase::TWO_PI * ModuleBase::IMAG_UNIT
                                 * (GlobalC::kv.kvec_d[ik] * static_cast<ModuleBase::Vector3<double>>(boxes[i])))
                          << "\t";
            }
            std::cout << std::endl;
        }
        ...
        for (size_t iat = 0; iat != GlobalC::ucell.nat; ++iat)
        {
            std::cout << GlobalC::ucell.atoms[GlobalC::ucell.iat2it[iat]].taud[GlobalC::ucell.iat2ia[iat]]
    ...
    auto test_matrix = []() {
        ...
        ModuleBase::ComplexMatrix cm
            = ModuleBase::ComplexMatrix(m) * exp(-ModuleBase::TWO_PI * ModuleBase::IMAG_UNIT * 1.0 / 3.0);
    ...
    std::ofstream ofs_mpi(test_dir.process + "time_" + ModuleBase::GlobalFunc::TO_STRING(GlobalV::MY_RANK),
                          std::ofstream::app);
    ...
    const std::vector<std::vector<std::vector<Numerical_Orbital_Lm>>> abfs_same_atom
        = Exx_Abfs::Construct_Orbs::abfs_same_atom(lcaos, this->kmesh_times, info.pca_threshold); // Peize Lin test
    ...
    auto print_psi2
        = [](const std::string &file_name, const std::vector<std::vector<std::vector<Numerical_Orbital_Lm>>> &orbs) {
    ...
    case Exx_Global::Hybrid_Type::HF:
        abfs_ccp
            = Conv_Coulomb_Pot_K::cal_orbs_ccp(this->abfs, Conv_Coulomb_Pot_K::Ccp_Type::Hf, {}, info.ccp_rmesh_times);
        break;
    case Exx_Global::Hybrid_Type::PBE0:
        abfs_ccp
            = Conv_Coulomb_Pot_K::cal_orbs_ccp(this->abfs, Conv_Coulomb_Pot_K::Ccp_Type::Ccp, {}, info.ccp_rmesh_times);
        break;
    case Exx_Global::Hybrid_Type::HSE:
        abfs_ccp = Conv_Coulomb_Pot_K::cal_orbs_ccp(this->abfs,
    ...
    auto print_psik = [](const std::string &file_name,
        ...
        for (size_t i = 0; i != orbs.size(); ++i)
        {
            for (size_t j = 0; j != orbs[i].size(); ++j)
            {
                for (size_t k = 0; k != orbs[i][j].size(); ++k)
                {
                    ...
                    std::ofstream ofs(file_name + "_" + ModuleBase::GlobalFunc::TO_STRING(i) + "_"
...
#error
    ...
    auto test_mll = [&]() {
        ...
        mll.cal_overlap_matrix(0,
                               0,
                               GlobalC::ucell.atoms[0].tau[0],
                               GlobalC::ucell.atoms[0].tau[0],
                               index_lcaos,
                               index_lcaos)
                .print(ofsS, 1E-10)
    ...
    auto PCA_test = [&]() {
        ...
        std::map<size_t, std::map<size_t, std::map<Abfs::Vector3_Order<double>, std::weak_ptr<ModuleBase::matrix>>>>
            Cws;
        std::map<size_t, std::map<size_t, std::map<Abfs::Vector3_Order<double>, std::weak_ptr<ModuleBase::matrix>>>>
            Vws;
        std::shared_ptr<ModuleBase::matrix> C = Abfs::DPcal_C(0,
                                                              0,
                                                              {0, 0, 0},
                                                              m_abfs_abfs,
                                                              m_abfslcaos_lcaos,
    ...
    auto overlap_test = [&]() {
        ...
        const ModuleBase::matrix m_overlap
            = m_lcaos_lcaos.cal_overlap_matrix(0, 0, {0, 0, 0}, {0, 0, 0}, index_lcaos, index_lcaos);
        ...
        const ModuleBase::matrix m_overlap_coulomb
            = m_lcaos_ccp.cal_overlap_matrix(0, 0, {0, 0, 0}, {0, 0, 0}, index_lcaos, index_lcaos);
...
void Exx_Lcao::cal_exx_ions(const Parallel_Orbitals &pv)
{
    ...
    if (atom_pairs_core_origin.empty())
        ...
        case Exx_Lcao::Distribute_Type::Kmeans2:
            atom_pairs_core_origin = Exx_Abfs::Parallel::Distribute::Kmeans::distribute_kmeans2(MPI_COMM_WORLD);
            break;
        case Exx_Lcao::Distribute_Type::Kmeans1:
            atom_pairs_core_origin
                = Exx_Abfs::Parallel::Distribute::Kmeans::distribute_kmeans1(MPI_COMM_WORLD, info.ccp_rmesh_times);
            break;
...
#error "TEST_EXX_LCAO"
    ...
    gettimeofday(&t_start, NULL);
    Cs = Abfs::cal_Cs(atom_centres_core,
                      m_abfs_abfs,
                      m_abfslcaos_lcaos,
                      index_abfs,
                      index_lcaos,
    ...
    schwarz.cal_max_pair_fock(atom_centres_core,
                              m_abfs_abfs,
                              m_abfslcaos_lcaos,
                              index_abfs,
                              index_lcaos,
...
#error "TEST_EXX_LCAO"
    ...
    auto print_LOCDM = [&]() {
                    ...
                    for (size_t is = 0; is != GlobalV::NSPIN; ++is)
                    {
                        ...
                        for (size_t iw1 = 0; iw1 != GlobalC::ucell.atoms[it1].nw; ++iw1)
                        {
                            for (size_t iw2 = 0; iw2 != GlobalC::ucell.atoms[it2].nw; ++iw2)
                                ofs << loc.DM[is][GlobalC::ucell.itiaiw2iwt(it1, ia1, iw1)]
                                             [GlobalC::ucell.itiaiw2iwt(it2, ia2, iw2)]
                                    << "\t";
                            ofs << std::endl;
                        }
                        ofs << std::endl;
        ...
        else
        {
            ...
            {
                std::ofstream
            ofs("LOC.DM_"+ModuleBase::GlobalFunc::TO_STRING(istep++)+"_"+ModuleBase::GlobalFunc::TO_STRING(is)); for(int
            T1=0; T1<GlobalC::ucell.ntype; T1++)
                {
    ...
    auto print_WFC = [&](std::complex<double> ***wfc_k_grid) {
        if (GlobalV::GAMMA_ONLY_LOCAL)
        {
            for (size_t ik = 0; ik != GlobalC::kv.nks; ++ik)
            {
                std::ofstream ofs("GlobalC::LOWF.WFC_GAMMA_" + ModuleBase::GlobalFunc::TO_STRING(istep) + "_"
                                  + ModuleBase::GlobalFunc::TO_STRING(ik));
        ...
        else
        {
            for (size_t ik = 0; ik != GlobalC::kv.nks; ++ik)
            {
                std::ofstream ofs("LOWF.wfc_k_grid_" + ModuleBase::GlobalFunc::TO_STRING(istep) + "_"
                                  + ModuleBase::GlobalFunc::TO_STRING(ik));
                for (size_t ib = 0; ib != GlobalV::NBANDS; ++ib)
                {
                    for (size_t iwt = 0; iwt != GlobalV::NLOCAL; ++iwt)
    ...
    auto print_Hexx = [&]() // Peize Lin test 2019-11-14
    {
        if (GlobalV::GAMMA_ONLY_LOCAL)
        {
            ...
            {
                std::ofstream ofs("Hexx_" + ModuleBase::GlobalFunc::TO_STRING(istep) + "_"
                         ...
                         std::vector<ModuleBase::ComplexMatrix> &wfc_k) // Peize Lin test 2019-11-14
    {
        if (GlobalV::GAMMA_ONLY_LOCAL)
        {
            for (int is = 0; is < GlobalV::NSPIN; ++is)
            {
                std::ofstream ofs("wfc_" + ModuleBase::GlobalFunc::TO_STRING(istep) + "_"
                                  + ModuleBase::GlobalFunc::TO_STRING(is) + "_"
                                  + ModuleBase::GlobalFunc::TO_STRING(GlobalV::MY_RANK));
    ...
    auto print_ekb = [&]() // Peize Lin test 2019-11-14
    {
        for (int ik = 0; ik < GlobalC::kv.nks; ++ik)
        {
            std::ofstream ofs("ekb_" + ModuleBase::GlobalFunc::TO_STRING(ik) + "_"
                                  + ModuleBase::GlobalFunc::TO_STRING(GlobalV::MY_RANK),
                              std::ofstream::app);
...
GlobalC::ucell.atoms[it1].tau[GlobalC::ucell.iat2ia[iat2]] ); const std::vector<Abfs::Atom_Info> adj2s = Abfs::get_adjs(
GlobalC::ucell.atoms[it2].tau[GlobalC::ucell.iat2ia[iat2]] ); for( const Abfs::Atom_Info & atom3 : adj1s )
        {
            ...
            for( const Abfs::Atom_Info & atom4 : adj2s )
            {
                ...
                const ModuleBase::matrix matrix_1342 = *Cs[iat1][iat3][atom3.box] * *Vps_gamma[iat1][iat2] *
                ...
                for( size_t iw1=0; iw1!=GlobalC::ucell.atoms[it1].nw; ++iw1 )
                    for( size_t iw2=0; iw2!=GlobalC::ucell.atoms[it2].nw; ++iw2 )
                        for( size_t iw3=0; iw3!=GlobalC::ucell.atoms[atom3.T].nw; ++iw3 )
                            ...
                            {
                                const double matrix_element_1342 = matrix_1342(
iw1*GlobalC::ucell.atoms[atom3.T].nw+iw3, iw2*GlobalC::ucell.atoms[atom4.T].nw+iw4 ); for( size_t is=0;
is!=GlobalV::NSPIN; ++is )
                                {
                                    Hexx_gamma[iat1][iat2][is](iw1,iw2) = matrix_element_1342 *
DM_Gamma[iat3][iat4][is](iw3,iw4); Hexx_gamma[iat1][iat4][is](iw1,iw4) = matrix_element_1342 *
DM_Gamma[iat3][iat2][is](iw3,iw2); Hexx_gamma[iat3][iat2][is](iw3,iw2) = matrix_element_1342 *
DM_Gamma[iat1][iat4][is](iw1,iw4); Hexx_gamma[iat3][iat4][is](iw3,iw4) = matrix_element_1342 *
DM_Gamma[iat1][iat2][is](iw1,iw2);
                                }
                            }
            }
        }
...
double Exx_Lcao::cal_energy(
    const std::vector<std::map<size_t, std::map<size_t, std::map<Abfs::Vector3_Order<int>, ModuleBase::matrix>>>>
        &HexxR) const
{
        ...
        for (const auto &HA: HexxR[is])
        {
            ...
            for (const auto &HB: HA.second)
            {
                ...
                for (const auto &HC: HB.second)
                {
                    ...
                    if (const ModuleBase::matrix *const DMr_ptr = static_cast<const ModuleBase::matrix *const>(
                            ModuleBase::GlobalFunc::MAP_EXIST(DM_para.DMr[is], iat1, iat2, box2)))
                    {
...
void Exx_Lcao::init_radial_table_ions(const std::set<size_t> &atom_centres_core,
...
#error "TEST_EXX_LCAO"
    ...
    for (const std::pair<size_t, size_t> &atom_pair: atom_pairs_core)
    {
        ...
        const ModuleBase::Vector3<double> &tau2 = GlobalC::ucell.atoms[it2].tau[ia2];
        const double Rcut
            = std::min(GlobalC::ORB.Phi[it1].getRcut() * info.ccp_rmesh_times + GlobalC::ORB.Phi[it2].getRcut(),
                       GlobalC::ORB.Phi[it1].getRcut() + GlobalC::ORB.Phi[it2].getRcut() * info.ccp_rmesh_times);
...
std::vector<std::map<size_t, std::map<size_t, std::map<Abfs::Vector3_Order<int>, ModuleBase::matrix>>>> Exx_Lcao::
    cal_Hexx() const
{
#ifdef __MKL
...
#pragma omp parallel
#endif
    {
        ...
        auto transform
            = [](const ModuleBase::matrix &m, const size_t n1, const size_t n2, const size_t n3) -> ModuleBase::matrix {
            assert(n1 * n2 * n3 == m.nr * m.nc);
            const auto length = sizeof(double) * n3;
                      ...
                      for (auto &m_tmp1: m_tmp[is])
                      {
                          ...
                          for (auto &m_tmp2: m_tmp1.second)
                          {
                              ...
                              for (auto &m_tmp3: m_tmp2.second)
                              {
                                  ...
                                  if (ModuleBase::matrix *const m_all_ptr = static_cast<ModuleBase::matrix *const>(
                                          ModuleBase::GlobalFunc::MAP_EXIST(m_all[is], iat1, iat2, box2)))
                                  {
        ...
        auto vector_empty
            = [](const std::vector<
                  std::map<size_t, std::map<size_t, std::map<Abfs::Vector3_Order<int>, ModuleBase::matrix>>>> &v)
            -> bool {
            for (const auto &i: v)
            {
                if (!i.empty())
...
#pragma omp for
        ...
        for (size_t i_atom_pair = 0; i_atom_pair < atom_pairs_core.size(); ++i_atom_pair)
        {
            ...
            auto add_Hexx_one = [&HexxR_tmp, iat1, iat2, this](const ModuleBase::matrix &Hexx_one,
                                                               const size_t is,
                                                               const size_t iat_I,
                                                               const size_t iat_J,
                ...
                if (iat1 != iat2)
                {
                    ...
                    if (ModuleBase::matrix *const HexxR_ptr
                        = static_cast<ModuleBase::matrix *const>(ModuleBase::GlobalFunc::MAP_EXIST(
                            HexxR_tmp[is],
                            iat_J,
                            iat_I,
                            Abfs::Vector3_Order<int>(-box_IJ) % this->Born_von_Karman_period)))
                        *HexxR_ptr += Hexx_one_T;
                    else
                        HexxR_tmp[is][iat_J][iat_I][Abfs::Vector3_Order<int>(-box_IJ) % this->Born_von_Karman_period]
                            = std::move(Hexx_one_T);
                }
                if (ModuleBase::matrix *const HexxR_ptr
                    = static_cast<ModuleBase::matrix *const>(ModuleBase::GlobalFunc::MAP_EXIST(
                    ...
                    *HexxR_ptr += Hexx_one;
                    ...
                    HexxR_tmp[is][iat_I][iat_J][Abfs::Vector3_Order<int>(box_IJ) % this->Born_von_Karman_period]
                        = std::move(Hexx_one);
            };
                ...
                for (const auto &Cp2s: Cps.at(iat2))
                {
                    ...
                    for (const auto &Cp1: Cp1s.second)
                    {
                        ...
                        for (const auto &Cp2: Cp2s.second)
                        {
                            ...
                            if (schwarz.screen(iat1, iat2, iat3, iat4, box3, box4))
                                continue;
                            ...
                            for (const auto &Vp: Vps.at(iat1).at(iat2))
                            {
                                ...
                                const Exx_Abfs::Screen::Cauchy::Info_Step info_step
                                    = cauchy.input_info(iat1, iat2, iat3, iat4, box2, box3, box4);
                                int cauchy_postcal;
                                if (!(cauchy_postcal = cauchy.postcalA(info_step)))
                                    continue;
                                ...
                                if (!(cauchy_postcal = cauchy.postcalB(info_step,
                                                                       VC_24_T,
                                                                       index_lcaos[it2].count_size,
                                                                       index_abfs[it1].count_size,
                                                                       index_lcaos[it4].count_size,
                                ...
                                for (size_t is = 0; is != GlobalV::NSPIN; ++is)
                                {
                                        ...
                                        const ModuleBase::matrix *const DM_ptr
                                            = static_cast<const ModuleBase::matrix *const>(
                                                ModuleBase::GlobalFunc::MAP_EXIST(
                                                    ...
                                                    iat3,
                                                    iat4,
                                                    Abfs::Vector3_Order<int>(box2 - box3 + box4)
                                                        % Born_von_Karman_period));
                                        if (DM_ptr)
                                        {
                                    ...
                                    case 3: {
                                        if (const ModuleBase::matrix *const DM_ptr
                                            = static_cast<const ModuleBase::matrix *const>(
                                                ModuleBase::GlobalFunc::MAP_EXIST(DM_para.DMr[is],
                                                                                  iat1,
                                                                                  iat4,
                                                                                  Abfs::Vector3_Order<int>(box2 + box4)
                                                                                      % Born_von_Karman_period)))
                                        {
                                        ...
                                        if (const ModuleBase::matrix *const DM_ptr
                                            = static_cast<const ModuleBase::matrix *const>(
                                                ModuleBase::GlobalFunc::MAP_EXIST(DM_para.DMr[is],
                                                                                  iat3,
                                                                                  iat2,
                                                                                  Abfs::Vector3_Order<int>(box2 - box3)
                                                                                      % Born_von_Karman_period)))
                                        {
