...
{
    hipblasCdotc(handle, n, reinterpret_cast<const hipblasComplex*>(x), incx, reinterpret_cast<const hipblasComplex*>(y), incy, reinterpret_cast<hipblasComplex*>(result));
}
static inline
...
{
    hipblasZdotc(handle, n, reinterpret_cast<const hipblasDoubleComplex*>(x), incx, reinterpret_cast<const hipblasDoubleComplex*>(y), incy, reinterpret_cast<hipblasDoubleComplex*>(result));
}
...
{
    hipblasCaxpy(handle, n, reinterpret_cast<const hipblasComplex*>(&alpha), reinterpret_cast<const hipblasComplex*>(x), incx, reinterpret_cast<hipblasComplex*>(y), incy);
}
static inline
...
{
    hipblasZaxpy(handle, n, reinterpret_cast<const hipblasDoubleComplex*>(&alpha), reinterpret_cast<const hipblasDoubleComplex*>(x), incx, reinterpret_cast<hipblasDoubleComplex*>(y), incy);
}
...
{
    hipblasCscal(handle, n, reinterpret_cast<const hipblasComplex*>(&alpha), reinterpret_cast<hipblasComplex*>(x), incx);
}
static inline
void scal(hipblasHandle_t& handle, const int& n, const std::complex<double>& alpha, std::complex<double> *x, const int& incx)
{
    hipblasZscal(handle, n, reinterpret_cast<const hipblasDoubleComplex*>(&alpha), reinterpret_cast<hipblasDoubleComplex*>(x), incx);
}
...
{
    hipblasSgemv(handle, GetHipblasOperation(trans), m, n, &alpha, A, lda, x, incx, &beta, y, incy);
}
static inline
void gemv(hipblasHandle_t& handle, const char& trans, const int& m, const int& n,
...
{
    hipblasDgemv(handle, GetHipblasOperation(trans), m, n, &alpha, A, lda, x, incx, &beta, y, incy);
}
static inline
void gemv(hipblasHandle_t& handle, const char& trans, const int& m, const int& n,
...
{
    hipblasCgemv(handle, GetHipblasOperation(trans), m, n, reinterpret_cast<const hipblasComplex*>(&alpha), 
...
{
    hipblasZgemv(handle, GetHipblasOperation(trans), m, n, reinterpret_cast<const hipblasDoubleComplex*>(&alpha), 
...
{
    hipblasSgemm(handle, GetHipblasOperation(transa), GetHipblasOperation(transb),
...
{
    hipblasDgemm(handle, GetHipblasOperation(transa), GetHipblasOperation(transb),
                m, n, k, &alpha, A, lda, B, ldb, &beta, C, ldc);
}
static inline
void gemm(hipblasHandle_t& handle, const char& transa, const char& transb, const int& m, const int& n, const int& k,
...
{
    hipblasCgemm(handle, GetHipblasOperation(transa), GetHipblasOperation(transb),
                m, n, k, 
...
{
    hipblasZgemm(handle, GetHipblasOperation(transa), GetHipblasOperation(transb),
                m, n, k,
...
{
    T** out = nullptr;
    hipMalloc(reinterpret_cast<void **>(&out), sizeof(T*) * batch_size);
    hipMemcpy(out, in, sizeof(T*) * batch_size, hipMemcpyHostToDevice);
    return out;
}
          ...
          const float& beta, float** C, const int& ldc, const int& batch_size)
{
    ...
    hipblasSgemmBatched(handle, GetHipblasOperation(transa), GetHipblasOperation(transb),
          ...
          const double& beta, double** C, const int& ldc, const int& batch_size)
{
    ...
    hipblasDgemmBatched(handle, GetHipblasOperation(transa), GetHipblasOperation(transb),
                       m, n, k, &alpha, d_A, lda, d_B, ldb, &beta, d_C, ldc, batch_size);
    hipFree(d_A);
    hipFree(d_B);
    hipFree(d_C);
          ...
          const std::complex<float>& beta, std::complex<float>** C, const int& ldc, const int& batch_size)
{
    ...
    hipblasCgemmBatched(handle, GetHipblasOperation(transa), GetHipblasOperation(transb),
                       m, n, k, 
                       reinterpret_cast<const hipblasComplex*>(&alpha), 
          ...
          const std::complex<double>& beta, std::complex<double>** C, const int& ldc, const int& batch_size)
{
    ...
    hipblasZgemmBatched(handle, GetHipblasOperation(transa), GetHipblasOperation(transb),
                       m, n, k, 
                       reinterpret_cast<const hipblasDoubleComplex*>(&alpha), 
