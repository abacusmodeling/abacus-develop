...
int pdlaebz(int *ijob, int *nitmax, int *n, int *mmax, int *minp, int *nbmin,
		double *abstol, double *reltol, double *pivmin, double *d__, double * e,
		double *e2, int *nval, double *ab, double *c__, int *mout, int *nab,
		double *work, int *iwork, int *info) {
	/* System generated locals */
	int nab_dim1, nab_offset, ab_dim1, ab_offset, i__1, i__2, i__3, i__4, i__5,
			i__6;
	...
	for (jit = 1; jit <= i__1; ++jit) {
		...
		for (ji = kf; ji <= i__2; ++ji) {
			...
			if (tmp1 < max(d__1, d__2)
					|| nab[ji + nab_dim1] >= nab[ji + (nab_dim1 << 1)]) {
