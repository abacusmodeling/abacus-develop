  ...
  //1
        for (bj=1; bj<b_n; bj++)
        {
            ...
            if (coord[0]==iarow1)
            {
                ...
                zlacpy_(&uplo,&n,&m,&B[pos],&ldb,&CU[0][cur_col],&ldb);
            }
            MPI_Bcast(CU,NB*loc_A.col_num,MPI_DOUBLE_COMPLEX,iarow1,comm_col);
...
//printf("1:A=%lf+i%lf,%lf+i%lf\n",A[0].real(),A[0].imag(),A[1].real(),A[1].imag());
  ...
  //printf("pos=%d,cur_col=%d,NB=%d,loc_A.col_num=%d,CU[0][0]=%lf+i%lf\n",pos,cur_col,NB,loc_A.col_num,CU[0][0].real(),CU[0][0].imag());          
   ztrsm_(&side,&uplo,&transb,&diag,&n,&m,&alpha,&CU[0][cur_col],&ldc,&A[pos],&lda);
 //printf("2:A=%lf+i%lf,%lf+i%lf\n",A[0].real(),A[0].imag(),A[1].real(),A[1].imag());
                for (i=0; i<m; i++)
                    for (j=0; j<n; j++)
                    {
            ...
            MPI_Bcast(CU_A,loc_A.row_num*NB,MPI_DOUBLE_COMPLEX,iacol1,comm_row);
...
//printf("1:%lf\n",alpha.real());
            ...
            //zgemm_(&transa,&transb,&n,&m,&k,&alpha,&CU[0][cur_col],&ldb,CU_A,&ldc,&beta,&A[pos],&lda);
            zgemm_(&transa,&transb,&n,&m,&k,&alpha,&CU[0][cur_col],&ldb,&CU_A[0][0],&ldc,&beta,&A[pos],&lda);
    ...
    else
    {
        ...
        for (bj=1; bj<b_n; bj++)
        {
            ...
            if (coord[1]==iacol1)
            {
                ...
                ldc=NB;
                pos=loc_A.row_pos*loc_B.col_num+loc_A.col_pos;
                zlacpy_(&uplo,&n,&m,&B[pos],&ldb,&CR[loc_A.row_pos][0],&ldc);
            }
            MPI_Bcast(CR,NB*loc_A.row_num,MPI_DOUBLE_COMPLEX,iacol1,comm_row);
            ...
            MPI_Bcast(CR_A,loc_A.col_num*NB,MPI_DOUBLE_COMPLEX,iarow1,comm_col);
            /*update A in iarow row*/
            transa='n';
            transb='n';
            ...
            zgemm_(&transa,&transb,&n,&m,&k,&alpha,&CR_A[0][0],&ldc,&CR[cur_row][0],&ldb,&beta,&A[pos],&lda);
            if (coord[0]==iarow1)
                loc_A.row_pos=loc_A.row_pos+NB;
            if (coord[1]==iacol1)
                loc_A.col_pos=loc_A.col_pos+NB;
