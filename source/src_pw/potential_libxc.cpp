	...
	const double * const rho_core_in)
{
		...
		auto process_vsigma = [&]()
		{
			...
			else // may need updates for SOC
			{
				...
				for( size_t ir=0; ir!=GlobalC::pw.nrxx; ++ir )
				{
					...
					const double neg = (ucell.magnet.lsign_
					                 && rho_in[1][ir]*ucell.magnet.ux_[0]+rho_in[2][ir]*ucell.magnet.ux_[1]+rho_in[3][ir]*ucell.magnet.ux_[2]<=0)
									 ? -1 : 1;
					if(amag > vanishing_charge)
					{
						for(int i=1;i<4;i++)
    ...
    etxc *= ucell.omega / GlobalC::pw.ncxyz;
    vtxc *= ucell.omega / GlobalC::pw.ncxyz;
	...
	auto cal_rho = [&]()
	{
		if(!finished_rho)
		{
			...
			else // may need updates for SOC
			{
				...
				{
					ucell.magnet.cal_ux(ucell.ntype);
				}
				for( size_t ir=0; ir!=GlobalC::pw.nrxx; ++ir )
				{
					const double amag = sqrt( pow(rho_in[1][ir],2) + pow(rho_in[2][ir],2) + pow(rho_in[3][ir],2) );
					const double neg = (ucell.magnet.lsign_ && rho_in[1][ir]*ucell.magnet.ux_[0]
					                    +rho_in[2][ir]*ucell.magnet.ux_[1]
					                    +rho_in[3][ir]*ucell.magnet.ux_[2]<=0)
					                   ? -1 : 1;
