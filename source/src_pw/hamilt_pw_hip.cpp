...
void Hamilt_PW::init_k(const int ik)
{
    ...
    for (int ig = 0;ig < GlobalC::wf.npw;ig++)
    {
		...
		// if(ig<20){
		// 	cout<<GR_index[ig]<<" "<< GlobalC::wf.igk(ik, ig) <<" "<<GlobalC::pw.ig2fftw[ GlobalC::wf.igk(ik, ig) ]<<endl;
		// }
    }
    // cout<<"init_K"<<endl;
#ifdef __ROCM
	...
	hipblasDoubleComplex *vkb_c)
{
	...
	CHECK_CUBLAS(hipblasZgeam(hpw_handle, HIPBLAS_OP_T, HIPBLAS_OP_T, nstart, nstart, &t_alpha, tmp_hc, nstart, &t_beta, tmp_hc, nstart, hc, nstart));
	...
	CHECK_CUBLAS(hipblasZgeam(hpw_handle, HIPBLAS_OP_T, HIPBLAS_OP_T, nstart, nstart, &t_alpha, tmp_hc, nstart, &t_beta, tmp_hc, nstart, sc, nstart));
	...
	*/
	...
	else
	{
		...
		for(int ib=0; ib<n_band; ib++)
		{
			...
			// 	evc(ib,ig) = evctmp(ib,ig);
			// }
			CHECK_CUDA(hipMemcpy(&evc[ib*dmax], &evctmp[ib*dmin], dmin*sizeof(hipblasDoubleComplex), hipMemcpyDeviceToDevice));
		}
...
void Hamilt_PW::h_1psi_cuda( const int npw_in, const hipblasComplex *psi,
                        hipblasComplex *hpsi, hipblasComplex *spsi, hipblasComplex *vkb_c)
{
    this->h_psi_cuda(psi, hpsi, vkb_c);

    int thread = 512;
    int block = (npw_in + thread - 1) / thread;
    hipLaunchKernelGGL(kernel_copy, dim3(thread), dim3(block), 0, 0, 
		npw_in, reinterpret_cast<float2*>(spsi), reinterpret_cast<const float2*>(psi));
    return;
}

void Hamilt_PW::h_1psi_cuda( const int npw_in, const hipblasDoubleComplex *psi,
	hipblasDoubleComplex *hpsi, hipblasDoubleComplex *spsi, hipblasDoubleComplex *vkb_c)
{
	this->h_psi_cuda(psi, hpsi, vkb_c);
	...
	int block = (npw_in + thread - 1) / thread;
	hipLaunchKernelGGL(kernel_copy, dim3(thread), dim3(block), 0, 0, 
		npw_in, reinterpret_cast<double2*>(spsi), reinterpret_cast<const double2*>(psi));
	return;
}
...
void Hamilt_PW::h_psi_cuda(const hipblasComplex *psi_in, hipblasComplex *hpsi, hipblasComplex *vkb_c, const int m)
{
 	...
 	if(GlobalV::T_IN_H)
	{
		...
		for(int ib = 0 ; ib < m; ++ib)
        {
			...
			// todo: template
            hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_get_tmhpsi<float, float2>), dim3(block), dim3(thread), 0, 0, 
				GlobalC::wf.npw, reinterpret_cast<float2*>(tmhpsi), reinterpret_cast<const float2*>(tmpsi_in), f_g2kin);
	...
	if(GlobalV::VL_IN_H)
	{
        ...
        for(int ib = 0 ; ib < m; ++ib)
        {
            ...
            int thread = 512;
            int block = (GlobalC::wf.npw + thread - 1) / thread;
            hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_add_tmhpsi<float2>), dim3(block), dim3(thread), 0, 0, 
				GlobalC::wf.npw, reinterpret_cast<float2*>(tmhpsi), reinterpret_cast<float2*>(f_porter), GR_index_d);
    ...
    if(GlobalV::VNL_IN_H)
	{
        if ( GlobalC::ppcell.nkb > 0)
        {
            ...
            // CHECK_CUDA(hipMemcpy(hpsi_cpu, hpsi, GlobalC::wf.npw*GlobalV::NPOL*sizeof(hipblasComplex), hipMemcpyDeviceToHost));

            // this->add_nonlocal_pp(hpsi_cpu, becp_cpu, m);

            // CHECK_CUDA(hipMemcpy(hpsi, hpsi_cpu, GlobalC::wf.npw*GlobalV::NPOL*sizeof(hipblasComplex), hipMemcpyHostToDevice));
...
void Hamilt_PW::h_psi_cuda(const hipblasDoubleComplex *psi_in, hipblasDoubleComplex *hpsi, hipblasDoubleComplex *vkb_c, const int m)
{
 	...
 	if(GlobalV::T_IN_H)
	{
        ...
        for(int ib = 0 ; ib < m; ++ib)
        {
            ...
            int thread = 512;
            int block = (GlobalC::wf.npw + thread - 1) / thread;
            hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_get_tmhpsi<double, double2>), dim3(block), dim3(thread), 0, 0, 
				GlobalC::wf.npw, reinterpret_cast<double2*>(tmhpsi), reinterpret_cast<const double2*>(tmpsi_in), d_g2kin);
	...
	if(GlobalV::VL_IN_H)
	{
        ...
        for(int ib = 0 ; ib < m; ++ib)
        {
            ...
            int thread = 512;
            int block = (GlobalC::wf.npw + thread - 1) / thread;
            hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_add_tmhpsi<double2>), dim3(block), dim3(thread), 0, 0, 
				GlobalC::wf.npw, reinterpret_cast<double2*>(tmhpsi), reinterpret_cast<double2*>(d_porter), GR_index_d);
    ...
    if(GlobalV::VNL_IN_H)
	{
        if ( GlobalC::ppcell.nkb > 0)
        {
            ...
            // CHECK_CUDA(hipMemcpy(becp_cpu, becp, GlobalV::NPOL*m*nkb*sizeof(hipblasDoubleComplex), hipMemcpyDeviceToHost));

            // CHECK_CUDA(hipMemcpy(hpsi_cpu, hpsi, GlobalC::wf.npw*GlobalV::NPOL*sizeof(hipblasDoubleComplex), hipMemcpyDeviceToHost));

            // this->add_nonlocal_pp(hpsi_cpu, becp_cpu, m);

            // CHECK_CUDA(hipMemcpy(hpsi, hpsi_cpu, GlobalC::wf.npw*GlobalV::NPOL*sizeof(hipblasDoubleComplex), hipMemcpyHostToDevice));
	...
	const int m)
{
    ...
    for (int it=0; it<GlobalC::ucell.ntype; it++)
    {
        ...
        for (int ia=0; ia<GlobalC::ucell.atoms[it].na; ia++)
        {
            ...
            hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_addpp<float, float2>), dim3(block), dim3(thread), 0, 0, 
				reinterpret_cast<float2*>(ps), f_cur_deeq, reinterpret_cast<const float2*>(becp), nproj, nprojx, sum, m, nkb);
	...
	const int m)
{
    ...
    for (int it=0; it<GlobalC::ucell.ntype; it++)
    {
        ...
        for (int ia=0; ia<GlobalC::ucell.atoms[it].na; ia++)
        {
            ...
            hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_addpp<double, double2>), dim3(block), dim3(thread), 0, 0, 
				reinterpret_cast<double2*>(ps), cur_deeq, reinterpret_cast<const double2*>(becp), nproj, nprojx, sum, m, nkb);
