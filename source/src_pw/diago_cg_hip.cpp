...
int Diago_CG_CUDA<T, T2, T3>::moved = 0;

// typedef hipblasComplex float2;
// typedef hipblasDoubleComplex hipblasDoubleComplex;

template<class T, class T2, class T3>
__global__ void kernel_normalization(T3 *data, int size, T norm)
{
...
template<class T, class T2, class T3>
__global__ void kernel_get_gredient(T3 *g, T3 *ppsi, int size, T lambda)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if(idx < size)
    {
...
template<class T, class T2, class T3>
__global__ void kernel_get_gammacg(int size, T3 *dst, const T3 *src, T gamma)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if(idx < size)
    {
...
template<class T, class T2, class T3>
__global__ void kernel_get_normacg(int size, T3 *dst, const T3 *src, T norma)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if(idx < size)
    {
...
template<class T, class T2, class T3>
Diago_CG_CUDA<T, T2, T3>::Diago_CG_CUDA()
{
    test_cg=0;
...
template<class T, class T2, class T3>
void Diago_CG_CUDA<T, T2, T3>::diag
(
...
template<class T, class T2, class T3>
void Diago_CG_CUDA<T, T2, T3>::calculate_gradient(
    ...
    T2 *g, T2 *ppsi)
{
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_precondition<T, T2, T3>), dim3(block), dim3(thread), 0, 0, 
        reinterpret_cast<T3*>(g), reinterpret_cast<const T3*>(hpsi), dim, precondition);
    // (3) PS|psi> : ppsi[i] = spsi[i]/precondition[i]
    hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_precondition<T, T2, T3>), dim3(block), dim3(thread), 0, 0, 
        reinterpret_cast<T3*>(ppsi), reinterpret_cast<const T3*>(spsi), dim, precondition);
...
template<class T, class T2, class T3>
void Diago_CG_CUDA<T, T2, T3>::orthogonal_gradient( const int &dim, const int &dmx,
                                    ...
                                    const hipblasComplex *eigenfunction, const int m)
{
    ...
    // CHECK_CUBLAS(hipblasZgemv(handle, trans1, dim, m, ONE, eigenfunction, dmx, sg, inc, ZERO, lagrange, inc));
    // hipblasComplex ONE, ZERO, NEG_ONE;

    // hipblasComplex ONE = make_hipComplex(1, 0);
    // hipblasComplex ZERO = make_hipComplex(0, 0);
    // hipblasComplex NEG_ONE = make_hipComplex(-1, 0);
                                    ...
                                    const hipblasDoubleComplex *eigenfunction, const int m)
{
    ...
    // ONE ZERO cufftcomplex?
    // CHECK_CUBLAS(hipblasZgemv(handle, trans1, dim, m, ONE, eigenfunction, dmx, sg, inc, ZERO, lagrange, inc));
    // hipblasDoubleComplex ONE, ZERO, NEG_ONE;
    // ONE.y = ZERO.x = ZERO.y = 0.0;
    // ONE.x = 1.0;
    // NEG_ONE.x = -1.0;
...
template<class T, class T2, class T3>
void Diago_CG_CUDA<T, T2, T3>::calculate_gamma_cg(
    const int iter,
    const int dim,
    const T *precondition,
    const T2 *g,
    ...
    const T2 *psi_m)
{
    ...
    int thread = 512;
    int block = (dim + thread - 1) / thread;
    hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_precondition_inverse<T, T2, T3>), dim3(block), dim3(thread), 0, 0, reinterpret_cast<T3*>(psg), reinterpret_cast<const T3*>(sg), dim, precondition);
    ...
    else
    {
        ...
        hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_get_gammacg<T, T2, T3>), dim3(block), dim3(thread), 0, 0, dim, reinterpret_cast<T3*>(cg), reinterpret_cast<const T3*>(g), gamma);
...
template<class T, class T2, class T3>
bool Diago_CG_CUDA<T, T2, T3>::update_psi(
    const int dim,
    T &cg_norm,
    T &theta,
    T2 *hcg,
...
//	cout << "\n overlap = "  << this->ddot(dim, psi_m, psi_m);
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_multi_add<T, T2, T3>), dim3(block), dim3(thread), 0, 0, 
        reinterpret_cast<T3*>(psi_m), reinterpret_cast<T3*>(psi_m), cost, reinterpret_cast<const T3*>(cg), sint_norm, dim);
...
template<class T, class T2, class T3>
void Diago_CG_CUDA<T, T2, T3>::schmit_orth
(
    const int& dim,
    const int& dmx,
    const int& m,     //end
...
)
{
    ...
    // hipblasComplex psi_norm_tmp(psi_norm);

    int thread = 512;
    int block = (dim + thread - 1) / thread;
    // float test_real = psi_m[0].real();
    hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_normalization<float, float2>), dim3(block), dim3(thread), 0, 0, reinterpret_cast<float2*>(psi_m), dim, psi_norm);

    // CHECK_CUBLAS(hipblasCaxpy(diag_handle, dim, &psi_norm_tmp, psi_m, 1, psi_m, 1));
...
)
{
    ...
    int thread = 512;
    int block = (dim + thread - 1) / thread;
    hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_normalization<double, double2>), dim3(block), dim3(thread), 0, 0, reinterpret_cast<double2*>(psi_m), dim, psi_norm);
...
template<class T, class T2, class T3>
float Diago_CG_CUDA<T, T2, T3>::ddot_real
(
    const int &dim,
...
template<class T, class T2, class T3>
double Diago_CG_CUDA<T, T2, T3>::ddot_real
(
    const int &dim,
...
template<class T, class T2, class T3>
hipblasComplex Diago_CG_CUDA<T, T2, T3>::ddot
(
    const int & dim,
    const hipblasComplex * psi_L,
    const hipblasComplex * psi_R
...
template<class T, class T2, class T3>
hipblasDoubleComplex Diago_CG_CUDA<T, T2, T3>::ddot
(
    const int & dim,
    const hipblasDoubleComplex * psi_L,
    const hipblasDoubleComplex * psi_R
