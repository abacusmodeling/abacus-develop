...
void Electrons::c_bands(const int &istep)
{
    ...
    for (int ik = 0;ik < GlobalC::kv.nks;ik++)
    {
        ...
        {
            CHECK_CUDA(hipMemcpy(h_diag, &GlobalC::wf.g2kin[0], GlobalC::wf.npw*sizeof(double), hipMemcpyHostToDevice));
            int thread = 512;
            int block = (GlobalC::wf.npw + thread - 1) / thread;
            hipLaunchKernelGGL(kernel_pred1, dim3(block), dim3(thread), 0, 0, h_diag, GlobalC::wf.npw);
            if(GlobalV::NPOL==2)
            {
                CHECK_CUDA(hipMemcpy(&h_diag[GlobalC::wf.npwx], h_diag, GlobalC::wf.npw*sizeof(double), hipMemcpyDeviceToDevice));
            }
		...
		if(GlobalV::NPOL==2)
        {
            ...
            if(diff_size > 0)
            {
                ...
                hipLaunchKernelGGL(kernel_set_ones, dim3(block), dim3(thread), 0, 0, &h_diag[GlobalC::wf.npw], diff_size);
                hipLaunchKernelGGL(kernel_set_ones, dim3(block), dim3(thread), 0, 0, &h_diag[GlobalC::wf.npw + GlobalC::wf.npwx], diff_size);
            }
        }
