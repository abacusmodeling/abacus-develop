    ...
    T2 *g, T2 *ppsi)
{
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_precondition<T, T2>), dim3(block), dim3(thread), 0, 0, g, hpsi, dim, precondition);
    // (3) PS|psi> : ppsi[i] = spsi[i]/precondition[i]
    hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_precondition<T, T2>), dim3(block), dim3(thread), 0, 0, ppsi, spsi, dim, precondition);
    ...
    // Update g !
    hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_get_gredient<T, T2>), dim3(block), dim3(thread), 0, 0, g, ppsi, dim, lambda);
    ...
    const T2 *psi_m)
{
    ...
    int thread = 512;
    int block = (dim + thread - 1) / thread;
    hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_precondition_inverse<T, T2>), dim3(block), dim3(thread), 0, 0, psg, sg, dim, precondition);
    ...
    else
    {
        ...
        hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_get_gammacg<T, T2>), dim3(block), dim3(thread), 0, 0, dim, cg, g, gamma);
        ...
        hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_get_normacg<T, T2>), dim3(block), dim3(thread), 0, 0, dim, cg, psi_m, norma);
    }
    ModuleBase::timer::tick("Diago_CG_CUDA","gamma_cg");
    return;
}
...
//	cout << "\n overlap = "  << this->ddot(dim, psi_m, psi_m);
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_multi_add<T, T2>), dim3(block), dim3(thread), 0, 0, psi_m, psi_m, cost, cg, sint_norm, dim);
...
//	cout << "\n overlap2 = "  << this->ddot(dim, psi_m, psi_m);
    ...
    else
    {
        ...
        // }
        hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_multi_add<T, T2>), dim3(block), dim3(thread), 0, 0, sphi, sphi, cost, scg, sint_norm, dim);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_multi_add<T, T2>), dim3(block), dim3(thread), 0, 0, hpsi, hpsi, cost, hcg, sint_norm, dim);
        ModuleBase::timer::tick("Diago_CG_CUDA","update_psi");
        return 0;
    }
}
...
)
{
    ...
    float2 *lagrange;
    CHECK_CUDA(hipMalloc((void**)&lagrange, (m+1)*sizeof(float2)));
    int inc=1;
    int mp1 = m+1;
    ...
    int thread = 512;
    int block = (dim + thread - 1) / thread;
    hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_normalization<float, float2>), dim3(block), dim3(thread), 0, 0, psi_m, dim, psi_norm);
...
)
{
    ...
    double2 *lagrange;
    CHECK_CUDA(hipMalloc((void**)&lagrange, (m+1)*sizeof(double2)));
    int inc=1;
    int mp1 = m+1;
    ...
    int thread = 512;
    int block = (dim + thread - 1) / thread;
    hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_normalization<double, double2>), dim3(block), dim3(thread), 0, 0, psi_m, dim, psi_norm);
