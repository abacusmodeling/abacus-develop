...
#ifdef __CUDA
    CHECK_CUDA(hipFree(GR_index_d));
    CHECK_CUDA(hipMalloc((void**)&GR_index_d, nrxx*sizeof(int)));
#endif
...
void Hamilt_PW::init_k(const int ik)
{
    ...
    // cout<<"init_K"<<endl;
#ifdef __CUDA
    CHECK_CUDA(hipMemcpy(this->GR_index_d, GR_index, GlobalC::pw.nrxx*sizeof(int), hipMemcpyHostToDevice));
    // delete [] GR_index_tmp;
#endif
	...
	double2 *vkb_c)
{
	...
	double2* sc;
	double2* hvec;
	CHECK_CUDA(hipMalloc((void**)&hc, nstart*nstart*sizeof(double2)));
	CHECK_CUDA(hipMalloc((void**)&tmp_hc, nstart*nstart*sizeof(double2)));
	CHECK_CUDA(hipMalloc((void**)&sc, nstart*nstart*sizeof(double2)));
	CHECK_CUDA(hipMalloc((void**)&hvec, nstart*n_band*sizeof(double2)));
	...
	double2* aux;
	CHECK_CUDA(hipMalloc((void**)&aux, dmax*nstart*sizeof(double2)));
	// double2* paux = aux;
	...
	CHECK_CUBLAS(hipblasZgemm(hpw_handle, trans1, trans2, nstart, nstart, dmin, &ONE, psi_c, dmax, aux, dmax, &ZERO, tmp_hc, nstart));
	...
	t_alpha.x = 1.0;
	CHECK_CUBLAS(cublasZgeam(hpw_handle, HIPBLAS_OP_T, HIPBLAS_OP_T, nstart, nstart, &t_alpha, tmp_hc, nstart, &t_beta, tmp_hc, nstart, hc, nstart));

	CHECK_CUBLAS(hipblasZgemm(hpw_handle, trans1, trans2, nstart, nstart, dmin, &ONE, psi_c, dmax, psi_c, dmax, &ZERO, tmp_hc, nstart));
	// sc=transpose(sc,false); // TODO: transpose
	CHECK_CUBLAS(cublasZgeam(hpw_handle, HIPBLAS_OP_T, HIPBLAS_OP_T, nstart, nstart, &t_alpha, tmp_hc, nstart, &t_beta, tmp_hc, nstart, sc, nstart));
	...
	CHECK_CUDA(hipMemcpy(h_hc.c, hc, nstart*nstart*sizeof(double2), hipMemcpyDeviceToHost));
	CHECK_CUDA(hipMemcpy(h_sc.c, sc, nstart*nstart*sizeof(double2), hipMemcpyDeviceToHost));

	GlobalC::hm.diagH_LAPACK(nstart, n_band, h_hc, h_sc, nstart, h_en, h_hvec);
	CHECK_CUDA(hipMemcpy(hvec, h_hvec.c, nstart*n_band*sizeof(double2), hipMemcpyHostToDevice));
	...
	*/
		...
		&& ( GlobalV::CALCULATION == "scf" || GlobalV::CALCULATION == "md" || GlobalV::CALCULATION == "relax")) //pengfei 2014-10-13
	{
		...
		ONE.y = ZERO.x = ZERO.y = 0.0;
		ONE.x = 1.0;
		CHECK_CUBLAS(hipblasZgemm(hpw_handle, transa, transb, dmax, n_band, nstart, &ONE, psi_c, dmax, hvec, n_band, &ZERO, evc, dmax));
	}
	else
	{
		// As the evc and psi may refer to the same matrix, we first
		...
		double2 *evctmp;
		CHECK_CUDA(hipMalloc((void**)&evctmp, n_band*dmin*sizeof(double2)));
		CHECK_CUBLAS(hipblasZgemm(hpw_handle, transa, transb, dmin, n_band, nstart, &ONE, psi_c, dmax, hvec, n_band, &ZERO, evctmp, dmin));
		...
		for(int ib=0; ib<n_band; ib++)
		{
			...
			// 	evc(ib,ig) = evctmp(ib,ig);
			// }
			CHECK_CUDA(hipMemcpy(&evc[ib*dmax], &evctmp[ib*dmin], dmin*sizeof(double2), hipMemcpyDeviceToDevice));
		}
                        ...
                        float2 *hpsi, float2 *spsi, float2 *vkb_c)
{
    ...
    hipLaunchKernelGGL(kernel_copy, dim3(thread), dim3(block), 0, 0, npw_in, spsi, psi);
    return;
}
...
{
    CHECK_CUDA(hipMemcpy(spsi, psi, dim*sizeof(float2), hipMemcpyDeviceToDevice));
    return;
}

void Hamilt_PW::s_1psi_cuda(const int dim, const double2 *psi, double2 *spsi)
{
    CHECK_CUDA(hipMemcpy(spsi, psi, dim*sizeof(double2), hipMemcpyDeviceToDevice));
    return;
}
...
void Hamilt_PW::h_psi_cuda(const float2 *psi_in, float2 *hpsi, float2 *vkb_c, const int m)
{
 	...
 	if(GlobalV::T_IN_H)
	{
		...
		for(int ib = 0 ; ib < m; ++ib)
        {
			...
			// todo: template
            hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_get_tmhpsi<float, float2>), dim3(block), dim3(thread), 0, 0, GlobalC::wf.npw, tmhpsi, tmpsi_in, f_g2kin);
	...
	if(GlobalV::VL_IN_H)
	{
        ...
        CHECK_CUDA(hipMalloc((void**)&f_porter, GlobalC::pw.nrxx * sizeof(float2)));

        CHECK_CUDA(hipMemcpy(d_vr_eff1, GlobalC::pot.vr_eff1, GlobalC::pw.nrxx*sizeof(double), hipMemcpyHostToDevice));
        ...
        for(int ib = 0 ; ib < m; ++ib)
        {
            ...
            CHECK_CUDA(hipMemset(f_porter, 0, GlobalC::pw.nrxx * sizeof(float2)));
            ...
            int thread = 512;
            int block = (GlobalC::wf.npw + thread - 1) / thread;
            hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_add_tmhpsi<float2>), dim3(block), dim3(thread), 0, 0, GlobalC::wf.npw, tmhpsi, f_porter, GR_index_d);
    ...
    if(GlobalV::VNL_IN_H)
	{
        if ( GlobalC::ppcell.nkb > 0)
        {
			...
			// double2 *d_vkb_c;
            // float2 *f_vkb_c;
            CHECK_CUDA(hipMalloc((void**)&becp, GlobalV::NPOL*m*nkb*sizeof(float2)));
            // CHECK_CUDA(hipMalloc((void**)&d_vkb_c, GlobalC::wf.npwx*nkb*sizeof(double2)));
			// CHECK_CUDA(hipMalloc((void**)&f_vkb_c, GlobalC::wf.npwx*nkb*sizeof(float2)));

            // CHECK_CUDA(hipMemcpy(d_vkb_c, GlobalC::ppcell.vkb.c, GlobalC::wf.npwx*nkb*sizeof(double2), hipMemcpyHostToDevice));
			// int thread = 512;
            ...
            {
                int inc = 1;
                CHECK_CUBLAS(hipblasCgemv(hpw_handle, transa, GlobalC::wf.npw, nkb, &ONE, vkb_c, GlobalC::wf.npwx, psi_in, inc, &ZERO, becp, inc));

            }
            else
            {
                int npm = GlobalV::NPOL * m;
                CHECK_CUBLAS(hipblasCgemm(hpw_handle, transa, transb, nkb, npm, GlobalC::wf.npw, &ONE, vkb_c, GlobalC::wf.npwx, psi_in, GlobalC::wf.npwx, &ZERO, becp, nkb));
            ...
            // CHECK_CUDA(hipMemcpy(becp_cpu, becp, GlobalV::NPOL*m*nkb*sizeof(float2), hipMemcpyDeviceToHost));

            // CHECK_CUDA(hipMemcpy(hpsi_cpu, hpsi, GlobalC::wf.npw*GlobalV::NPOL*sizeof(float2), hipMemcpyDeviceToHost));

            // this->add_nonlocal_pp(hpsi_cpu, becp_cpu, m);

            // CHECK_CUDA(hipMemcpy(hpsi, hpsi_cpu, GlobalC::wf.npw*GlobalV::NPOL*sizeof(float2), hipMemcpyHostToDevice));
...
void Hamilt_PW::h_psi_cuda(const double2 *psi_in, double2 *hpsi, double2 *vkb_c, const int m)
{
 	...
 	if(GlobalV::T_IN_H)
	{
        ...
        for(int ib = 0 ; ib < m; ++ib)
        {
            ...
            int thread = 512;
            int block = (GlobalC::wf.npw + thread - 1) / thread;
            hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_get_tmhpsi<double, double2>), dim3(block), dim3(thread), 0, 0, GlobalC::wf.npw, tmhpsi, tmpsi_in, d_g2kin);
	...
	if(GlobalV::VL_IN_H)
	{
        ...
        // CHECK_CUDA(hipMalloc((void**)&d_GR_index, GlobalC::wf.npwx * sizeof(int)));
        CHECK_CUDA(hipMalloc((void**)&d_vr_eff1, GlobalC::pw.nrxx * sizeof(double)));
        CHECK_CUDA(hipMalloc((void**)&d_porter, GlobalC::pw.nrxx * sizeof(double2)));
        ...
        for(int ib = 0 ; ib < m; ++ib)
        {
            ...
            // if(NSPIN!=4){
            // ZEROS( UFFT.porter, pw.nrxx);
            CHECK_CUDA(hipMemset(d_porter, 0, GlobalC::pw.nrxx * sizeof(double2)));
			// cout<<"m:"<<m<<endl;
			// cout<<"grindex:"<<endl;
            ...
            int thread = 512;
            int block = (GlobalC::wf.npw + thread - 1) / thread;
            hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_add_tmhpsi<double2>), dim3(block), dim3(thread), 0, 0, GlobalC::wf.npw, tmhpsi, d_porter, GR_index_d);
    ...
    if(GlobalV::VNL_IN_H)
	{
        if ( GlobalC::ppcell.nkb > 0)
        {
            ...
            double2 *becp;
            // double2 *d_vkb_c;
            CHECK_CUDA(hipMalloc((void**)&becp, GlobalV::NPOL*m*nkb*sizeof(double2)));
            // CHECK_CUDA(hipMalloc((void**)&d_vkb_c, GlobalC::wf.npwx*nkb*sizeof(double2)));
            ...
            {
                int inc = 1;
                CHECK_CUBLAS(hipblasZgemv(hpw_handle, transa, GlobalC::wf.npw, nkb, &ONE, vkb_c, GlobalC::wf.npwx, psi_in, inc, &ZERO, becp, inc));

            }
            else
            {
                int npm = GlobalV::NPOL * m;
                CHECK_CUBLAS(hipblasZgemm(hpw_handle, transa, transb, nkb, npm, GlobalC::wf.npw, &ONE, vkb_c, GlobalC::wf.npwx, psi_in, GlobalC::wf.npwx, &ZERO, becp, nkb));
            ...
            // CHECK_CUDA(hipMemcpy(becp_cpu, becp, GlobalV::NPOL*m*nkb*sizeof(double2), hipMemcpyDeviceToHost));

            // CHECK_CUDA(hipMemcpy(hpsi_cpu, hpsi, GlobalC::wf.npw*GlobalV::NPOL*sizeof(double2), hipMemcpyDeviceToHost));

            // this->add_nonlocal_pp(hpsi_cpu, becp_cpu, m);

            // CHECK_CUDA(hipMemcpy(hpsi, hpsi_cpu, GlobalC::wf.npw*GlobalV::NPOL*sizeof(double2), hipMemcpyHostToDevice));
	...
	const int m)
{
    ...
    float2 *ps;
    CHECK_CUDA(hipMalloc((void**)&ps, nkb * GlobalV::NPOL * m * sizeof(float2)));
    CHECK_CUDA(hipMemset(ps, 0, GlobalV::NPOL * m * sizeof(float2)));
    ...
    for (int it=0; it<GlobalC::ucell.ntype; it++)
    {
        ...
        {
            CHECK_CUDA(hipMemcpy(cur_deeq, &(GlobalC::ppcell.deeq(GlobalV::CURRENT_SPIN, iat, 0, 0)),
                nprojx*nprojx*sizeof(double), hipMemcpyHostToDevice));
            ...
            hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_addpp<float, float2>), dim3(block), dim3(thread), 0, 0, ps, f_cur_deeq, becp, nproj, nprojx, sum, m, nkb);
	...
	const int m)
{
    ...
    // ZEROS(ps, GlobalV::NPOL * m * nkb);
    double2 *ps;
    CHECK_CUDA(hipMalloc((void**)&ps, nkb * GlobalV::NPOL * m * sizeof(double2)));
    CHECK_CUDA(hipMemset(ps, 0, GlobalV::NPOL * m * sizeof(double2)));
    ...
    for (int it=0; it<GlobalC::ucell.ntype; it++)
    {
        ...
        for (int ia=0; ia<GlobalC::ucell.atoms[it].na; ia++)
        {
            ...
            hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_addpp<double, double2>), dim3(block), dim3(thread), 0, 0, ps, cur_deeq, becp, nproj, nprojx, sum, m, nkb);
