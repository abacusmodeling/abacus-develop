    ...
    else
    {
        ...
        do
        {
	   		if(GlobalV::KS_SOLVER=="cg")
            {
                ...
                CHECK_CUDA(hipMalloc((void**)&d_wf_evc, GlobalV::NBANDS * GlobalC::wf.npwx * GlobalV::NPOL * sizeof(double2)));
                CHECK_CUDA(hipMalloc((void**)&d_wf_ekb, GlobalV::NBANDS * sizeof(double)));

                CHECK_CUDA(hipMemcpy(d_wf_evc, 
                            GlobalC::wf.evc[ik0].c, 
                            GlobalV::NBANDS * GlobalC::wf.npwx * GlobalV::NPOL * sizeof(double2), hipMemcpyHostToDevice));
				...
				if(GlobalV::NPOL==1)
				{
                    // CHECK_CUDA(hipMalloc((void**)&d_wf_evc, GlobalV::NBANDS * GlobalC::wf.npwx * sizeof(double2)));
                    // CHECK_CUDA(hipMalloc((void**)&d_wf_ekb, GlobalV::NBANDS * sizeof(double)));
                    ...
                    // CHECK_CUDA(hipMemcpy(d_wf_evc, GlobalC::wf.evc[ik0].c, GlobalV::NBANDS * GlobalC::wf.npwx * sizeof(double2), hipMemcpyHostToDevice));
                    ...
                    if(iter < 0)
                    {
                        CHECK_CUDA(hipMalloc((void**)&f_wf_evc, GlobalV::NBANDS * GlobalC::wf.npwx * sizeof(float2)));
                        ...
                        hipLaunchKernelGGL(hamilt_cast_d2f, dim3(block), dim3(thread), 0, 0, f_wf_evc, d_wf_evc, GlobalV::NBANDS * GlobalC::wf.npwx);
                        hipLaunchKernelGGL(hamilt_cast_d2f, dim3(block3), dim3(thread), 0, 0, f_precondition, d_precondition, DIM_CG_CUDA);
                        // add vkb_c parameter
                        hipLaunchKernelGGL(hamilt_cast_d2f, dim3(block4), dim3(thread), 0, 0, f_vkb_c, d_vkb_c, GlobalC::wf.npwx*nkb);
                        // CHECK_CUFFT(hipfftPlan3d(&GlobalC::UFFT.fft_handle, GlobalC::pw.nx, GlobalC::pw.ny, GlobalC::pw.nz, HIPFFT_C2C));
                        ...
                        hipLaunchKernelGGL(hamilt_cast_f2d, dim3(block), dim3(thread), 0, 0, d_wf_evc, f_wf_evc, GlobalV::NBANDS * GlobalC::wf.npwx);
                        hipLaunchKernelGGL(hamilt_cast_f2d, dim3(block2), dim3(thread), 0, 0, d_wf_ekb, f_wf_ekb, GlobalV::NBANDS);
                    ...
                    // to cpu
                    CHECK_CUDA(hipMemcpy(GlobalC::wf.evc[ik0].c, d_wf_evc, GlobalV::NBANDS * GlobalC::wf.npwx * sizeof(double2), hipMemcpyDeviceToHost));
				...
				{
                    // to gpu
                    // CHECK_CUDA(hipMalloc((void**)&d_wf_evc, GlobalV::NBANDS * DIM_CG_CUDA2 * sizeof(double2)));
                    // CHECK_CUDA(hipMalloc((void**)&d_wf_ekb, GlobalV::NBANDS * sizeof(double)));
                    CHECK_CUDA(hipMalloc((void**)&d_precondition, DIM_CG_CUDA2 * sizeof(double)));

                    // CHECK_CUDA(hipMemcpy(d_wf_evc, GlobalC::wf.evc[ik0].c, GlobalV::NBANDS * DIM_CG_CUDA2 * sizeof(double2), hipMemcpyHostToDevice));
                    ...
                    // to cpu
                    CHECK_CUDA(hipMemcpy(GlobalC::wf.evc[ik0].c, d_wf_evc, GlobalV::NBANDS * DIM_CG_CUDA2 * sizeof(double2), hipMemcpyDeviceToHost));
                    CHECK_CUDA(hipMemcpy(GlobalC::wf.ekb[ik], d_wf_ekb, GlobalV::NBANDS * sizeof(double), hipMemcpyDeviceToHost));
	...
	double2* hvec)  // nstart * n_band
{
    ...
    double2 *sdum;
    double2 *hdum;
    CHECK_CUDA(hipMalloc((void**)&sdum, nstart*ldh*sizeof(double2)));
    // CHECK_CUDA(hipMalloc((void**)&hdum, nstart*ldh*sizeof(double2)));

    // sdum = sc;
    CHECK_CUDA(hipMemcpy(sdum, sc, nstart*ldh*sizeof(double2), hipMemcpyDeviceToDevice));
    ...
    if (all_eigenvalues) // nstart = nbands
    {
        ...
        // hvec = hc;
        // repalce "=" with memcpy!
        CHECK_CUDA(hipMemcpy(hvec, hc, nstart*nbands*sizeof(double2), hipMemcpyDeviceToDevice));
        hipDeviceSynchronize();
        // use cusolver
        ...
        CHECK_CUDA(hipMalloc((void**)&cusolver_work, cusolver_lwork*sizeof(double2)));
        CHECK_CUSOLVER(cusolverDnZhegvd(
            ...
            cusolver_lwork,
            device_info));
        CHECK_CUDA(hipFree(cusolver_work));
    ...
    else // nstart != nbands
    {
        ...
        CHECK_CUDA(hipMalloc((void**)&hdum, nstart*ldh*sizeof(double2)));
        // hdum = hc;
        CHECK_CUDA(hipMemcpy(hdum, hc, nstart*ldh*sizeof(double2), hipMemcpyDeviceToDevice));
        hipDeviceSynchronize();
        int cusolver_lwork = 0;
