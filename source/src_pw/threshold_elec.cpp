...
void Threshold_Elec::set_diag_thr_e(void) const
{
	ModuleBase::TITLE("Threshold_Elec","set_diag_thr_e");
    //========================================================================
    // setup diag_thr_e see setup.f90 Page 5/14
    // setup diag_thr_e, the convergence threshold for eigenvalues
    ...
    else if(GlobalV::CALCULATION=="scf" || GlobalV::CALCULATION=="md" || GlobalV::CALCULATION=="relax" || GlobalV::CALCULATION=="scf-sto")//qianrui 2021-2-20
    {
        if (abs(GlobalV::DIAG_THR_E - 1.0e-2) < 1.0e-10)
        {
            if (GlobalC::pot.init_chg == "file")
            {
                ...
                // set a strict diag_thr_e in the input file ()diago_the_init
                //======================================================
                GlobalV::DIAG_THR_E = 1.0e-5;
            }
            else
            {
                //=======================================================
...
void Threshold_Elec::update_diag_thr_e(const int &iter)
{
    //======================================================
    ...
    if (iter > 1)
    {
        if (iter == 2)
        {
            GlobalV::DIAG_THR_E = 1.e-2;
        }

		//----------------------------
		// GlobalV::DIAG_THR_E changes in CG Method.
