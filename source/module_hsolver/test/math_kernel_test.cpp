...
class TestModuleHsolverMathKernel : public ::testing::Test
{
  protected:
    ...
    using synchronize_memory_op_gpu = psi::memory::synchronize_memory_op<std::complex<double>, psi::DEVICE_CPU, psi::DEVICE_GPU>;
...
// haozhihan add
    ...
    using constantvector_addORsub_constantVector_op_gpu = hsolver::constantvector_addORsub_constantVector_op<double, psi::DEVICE_GPU>;
    ...
    const std::vector<std::complex<double> > output_vector_div_constant_op = {
      {-0.11893203,-0.13492526}, {-0.40314756, 0.07734553},
      {0.61158728, -0.45754102}, {-0.54274745,-0.09682102},
      {0.06892412,  0.14837423}, {0.30232967,  0.49411249},
      {0.40018669,  0.11908754}, {0.27482480,  0.13000269},
      {0.09735741,  0.26480576}, {-0.43036571, 0.22365283},
      {0.62055009, -0.42136736}, {-0.05023313,-0.24673028}
    };
    ...
    const std::vector<std::complex<double> >output_vector_mul_vector_op = {
      {0.42788105,  0.48541979}, {1.64544237, -0.31568492},
      {-7.45843754, 5.57981051}, {-1.26986738,-0.22653235},
      {1.27513170,  2.74499965}, {-4.18442883,-6.83882118},
      {-6.57030931,-1.95519245}, {-0.80491673,-0.38075656},
      {0.20298579,  0.55210799}, {-1.93161921, 1.00382555},
      {5.67522380, -3.85360363}, {-0.75082970,-3.68785357}
    };

    const std::vector<std::complex<double> >output_vector_div_vector_op = {
      {1.00000000,  1.13447369}, {2.98793242, -0.57324718},
      {-1.51703084, 1.13492197}, {-7.01717974,-1.25179862},
      {0.11269711,  0.24260516}, {-0.66077063,-1.07993047},
      {-0.73733499,-0.21941613}, {-2.83848271,-1.34271145},
      {1.41253043,  3.84198987}, {-2.90054225, 1.50735632},
      {2.05256102, -1.39373474}, {-0.10166335,-0.49934031}
    };
    ...
    const std::vector<std::complex<double> >output_constantvector_addORsub_constantVector_op = {
      {-5.05571797, -5.64586374}, {-14.76279273,4.05181248},
      {21.81709620,-17.11884992}, {-4.90588639,-3.57826786},
      {2.48516640,   4.68956570}, {12.00198564,17.92778274},
      {15.60706781,  7.16903816}, {16.93937507,16.63938857},
      {2.80304798,   9.88869860}, {-6.07087895,10.26185851},
      {28.74131667,-12.89639182}, {-2.39253058,-8.97849535}
    };
    ...
    const std::vector<std::complex<double> >output_axpy_op = {
      {-1.04187063,  2.57843137}, {4.36025552, 5.18794423},
      {-11.42394822,-4.75054595}, {6.50906977, 8.24708166},
      {1.46770822,  -2.33007099}, {4.53333119,-8.23539294},
      {-1.41856581, -5.83818462}, {1.10278588,-2.14220619},
      {2.67173827,  -3.46052805}, {8.79651547, 4.55949747},
      {-9.50076036, -4.51100042}, {-3.10745172,2.72118808}
    };
    ...
    const std::vector<std::complex<double> >output_scal_op = {
      {-0.87403307,  2.74844882}, {4.38946833,  4.90517834},
      {-11.33678411,-4.63461167}, {3.14637745,  8.26155093},
      {1.47152165,  -2.17179400}, {4.29982696, -8.23346099},
      {-1.66408648, -6.48503924}, {-0.47976752,-4.85136318},
      {2.83788058,  -3.52331200}, {6.62574354,  4.07239273},
      {-10.91333952,-5.05628302}, {-2.97811808, 2.72623036}
    };
...
{
  gemv_op_cpu()(cpu_ctx, 'C', 2,  3, &ModuleBase::ONE, A_gemv.data(), 2, X_gemv.data(), 1, &ModuleBase::ONE, Y_gemv.data(), 1);
  char trans = 'C';
  int inc = 1;
  int row = 2;
  int col = 3;
  zgemv_(&trans, &row,  &col, &ModuleBase::ONE, A_gemv.data(), &row, X_gemv.data(), &inc, &ModuleBase::ONE, Y_test_gemv.data(), &inc);
...
TEST_F(TestModuleHsolverMathKernel, constantvector_addORsub_constantVector_op_gpu)
{
  ...
  // run
  constantvector_addORsub_constantVector_op_gpu()(gpu_ctx, dim, output_dev, input1_dev, constant1, input2_dev, constant2);
