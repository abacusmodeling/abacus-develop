    ...
    while (istep <= GlobalV::RELAX_NMAX && !stop)
    {
        ...
        if (GlobalV::OUT_LEVEL == "ie" || GlobalV::OUT_LEVEL == "m")
        {
            ...
            /*          std::cout << " ---------------------------------------------------------" << std::endl;
                        ...
                        GlobalV::ofs_running << " ---------------------------------------------------------" <<
               std::endl; if(GlobalV::CALCULATION=="relax")
                        {
                        ...
                        else if(GlobalV::CALCULATION=="cell-relax")
                        {
                            ...
                            GlobalV::ofs_running << " RELAX IONS : " << force_step << " (in total: " << istep << ")" <<
               std::endl;
                        }
                        else if(GlobalV::CALCULATION=="scf")
                        {
        ...
        if (GlobalV::deepks_scf)
        {
            ...
            std::cout << "E_delta_NN= " << std::setprecision(8) << GlobalC::ld.E_delta << " Ry"
                      << " = " << std::setprecision(8) << GlobalC::ld.E_delta * ModuleBase::Ry_to_eV << " eV"
                      << std::endl;
        }
#endif
        ...
        if (GlobalV::OUT_LEVEL == "i")
        {
            ...
            std::cout << std::setiosflags(ios::scientific) << " " << std::setw(7) << ss.str() << std::setw(5)
                      << ELEC_scf::iter << std::setw(18) << std::setprecision(6)
                      << GlobalC::en.etot * ModuleBase::Ry_to_eV;

            std::cout << std::setprecision(2) << std::setiosflags(ios::scientific) << std::setw(10)
                      << IMM.get_ediff() * ModuleBase::Ry_to_eV * 1000 << std::setw(10)
                      << IMM.get_largest_grad() * ModuleBase::Ry_to_eV / ModuleBase::BOHR_TO_A;
...
bool LOOP_ions::force_stress(const int &istep, int &force_step, int &stress_step, Record_adj &ra)
{
