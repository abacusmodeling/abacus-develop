    ...
    Local_Orbital_Charge &loc)
{
	...
	this->before_solver(istep, wfc_gamma, wfc_k, loc);
	// do self-interaction calculations / nscf/ tddft, etc. 
	this->solver(istep, wfc_gamma, wfc_k, loc);
    ...
    Local_Orbital_Charge &loc)
{
	...
	loc.allocate_dm_wfc(GlobalC::GridT, wfc_gamma, wfc_k);
    ...
    Local_Orbital_Charge &loc)
{
			...
			|| GlobalV::CALCULATION=="relax" || GlobalV::CALCULATION=="cell-relax") //pengfei 2014-10-13
	{
		...
		{
			ELEC_scf es;
            es.scf(istep - 1, wfc_gamma, wfc_k, loc);
        }
	...
	{
		ELEC_nscf::nscf(GlobalC::UHM, wfc_gamma, loc.dm_gamma, wfc_k, loc.dm_k);
	}
