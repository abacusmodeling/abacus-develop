                            ...
                            const bool &quadratic)
{
    ...
    if(block)
    {
        ...
        {
            GlobalV::ofs_running << "Adding potential to prevent interaction between lower and upper part of unit cell" << std::endl;
        }
    ...
    for (int ir = 0; ir < rho_basis->nrxx; ++ir)
    {
        ...
        if(linear)
        {
            value += rho_surface * ModuleBase::e2 * (mopopla(zgate, gatepos, true) + 1.0/6.0) * cell.lat0 / Efield::bmod;
...
void Gatefield::compute_force(const UnitCell &cell, ModuleBase::matrix &fgate)
{
    ...
    for(int it=0; it<cell.ntype; ++it)
    {
        ...
        for(int ia=0; ia<cell.atoms[it].na; ++ia)
        {
            ...
            {
                fgate(iat, jj) = - zval * ModuleBase::e2 * rho_surface * Efield::bvec[jj] / Efield::bmod * (fac - 2.0 * pos);
            }
            ++iat;
        }
    }
