...
void LCAO_gen_fixedH::build_Nonlocal_beta(const bool& calc_deri) //update by liuyu 2021-04-07
{
	...
	for(int T1=0; T1<ucell.ntype; ++T1)
        for(int T2=0; T2<ucell.ntype; ++T2)
            Rcut(T1,T2) = ORB.Phi[T1].getRcut() + ORB.Phi[T2].getRcut();
	
    for (int T0 = 0; T0 < ucell.ntype; T0++)
    {
		const Atom* const atom0 = &ucell.atoms[T0]; 
        for (int I0 =0; I0< atom0->na; I0++)
        {
            ...
            for (int ad1=0; ad1<GridD.getAdjacentNum()+1 ; ++ad1)
            {
				...
				const Atom* const atom1 = &ucell.atoms[T1];
				...
				for (int ad2=0; ad2 < GridD.getAdjacentNum()+1 ; ad2++)
				{
					...
					const Atom* const atom2 = &ucell.atoms[T2];
					const int nw2_tot = atom2->nw*NPOL;

					const Vector3<double> dtau = tau2 - tau1;
					const double distance = dtau.norm() * ucell.lat0;
					const double rcut = Rcut(T1,T2);
					//double rcut = ORB.Phi[T1].getRcut() + ORB.Phi[T2].getRcut();
					if(distance < rcut)
					{
						// ------------- enter the nnr increaing zone --------------
